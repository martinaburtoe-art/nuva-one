...
async function runCheck(fn) {
  let r = await fn();
  if (!r.ok && RETRY_MS > 0) { await sleep(RETRY_MS); r = await fn(); } // re-verifica: evita falsos positivos
  return r;
}

// ---------- Store (Supabase REST, service_role) ----------
...
async function main() {
  ...
  if (!open) { // sin registro: alerta como máximo 1 vez por hora (cron cada 10 min)
  ...
  for (const r of results) {
    ...
    if (!r.ok && !cur) {
      ...
      await sb('ops_incidents', { method: 'POST', body: JSON.stringify({
        fingerprint: r.name, check_name: r.name, severity: r.severity, summary: r.summary.slice(0, 500),
        details: r.details, notified_at: sent ? now : null }) });
    } else if (!r.ok && cur) {
