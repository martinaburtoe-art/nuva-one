...
const base = { id: "ADJ-1", period: "2026-08", reason: "Corrección conciliación", entityId: "DTE-1", impact: 190, requestedBy: "user-1", state: "requested" as const };
...
describe("tax adjustment workflow", () => {
  ...
  it("allows approved -> applied -> revalidated", () => {
    ...
    const revalidated = transitionTaxAdjustment({ ...base, state: "applied" }, "revalidated", "system");
    expect(revalidated.allowed).toBe(true);
  });
  it("rejects invalid transitions", () => {
    const result = transitionTaxAdjustment({ ...base, state: "requested" }, "revalidated", "user-1");
    expect(result.allowed).toBe(false);
