...
export async function processStudioCallback(args: { request: Request; jobId: string; step: number; token: string }) {
  ...
  const releaseClaim = async () => { await (admin as any).from("nuva_studio_job_callbacks").update({ status: "pending", payload: null, received_at: null }).eq("id", callback.data.id).eq("status", "received"); };
  ...
  const cycle = await (admin as any).from("nuva_studio_campaign_cycles").select("id,campaign_id,cycle_number").eq("studio_job_id", args.jobId).maybeSingle();
  if (!cycle.error && cycle.data && ["completed", "partial", "failed", "dead_letter", "cancelled"].includes(String(resumed?.status))) {
    ...
    await (admin as any).from("nuva_studio_campaign_cycles").update({ status: cycleStatus, completed_at: new Date().toISOString(), learnings: { jobStatus: resumed.status, completedSteps: resumed.result?.completed?.length ?? 0, media: resumed.result?.media ?? [] } }).eq("id", cycle.data.id).eq("status", "running");
    const campaign = await (admin as any).from("nuva_studio_campaigns").select("id,cadence_hours,max_cycles,cycles_completed,status").eq("id", cycle.data.campaign_id).maybeSingle();
