...
export async function processStudioCallback(args: { request: Request; jobId: string; step: number; token: string }) {
  ...
  const callbackRow = callback.data;
  if (callbackRow.status !== "pending") return { status: 200, body: { ok: true, duplicate: true } };
  if (new Date(String(callbackRow.expires_at)).getTime() <= Date.now()) { await admin.from("nuva_studio_job_callbacks").update({ status: "expired" }).eq("id", callbackRow.id).eq("status", "pending"); return { status: 410, body: { error: "Callback expired" } }; }
  ...
  const claimed = await admin.from("nuva_studio_job_callbacks").update({ status: "received", payload, received_at: new Date().toISOString() }).eq("id", callbackRow.id).eq("status", "pending").select("id").maybeSingle();
  if (claimed.error || !claimed.data) return { status: 200, body: { ok: true, duplicate: true } };
  const releaseClaim = async () => { await admin.from("nuva_studio_job_callbacks").update({ status: "pending", payload: null, received_at: null }).eq("id", callbackRow.id).eq("status", "received"); };
  ...
  if (!resumed) return { status: 500, body: { error: "Job could not be resumed after callback" } };
  ...
  return { status: 200, body: { ok: true, resumed: true, jobId: args.jobId, status: resumed.status } };
}
