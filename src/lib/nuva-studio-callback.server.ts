...
export async function processStudioCallback(args: { request: Request; jobId: string; step: number; token: string }) {
  ...
  if (!failed && storagePath) {
    const assetInsert = await (admin as any).from("ai_asset_library").insert({
      ...
      asset_type: "video", title: `Nüva Studio · video · paso ${args.step + 1}`, storage_path: storagePath,
      public_url: publicUrl ?? null, metadata: { source: "nuva-studio-callback", capability: "video", step: args.step, model: model ?? null, mimeType: mimeType ?? null },
    });
