...
export async function upsertStudioJobStep(args: { supabase: SupabaseClient<Database>; jobId: string; step: StudioExecutionStep; status: StudioStepStatus; attempts: number; result?: unknown; error?: unknown }) { const now = new Date().toISOString(); const row = { job_id: args.jobId, step: args.step.index, capability: args.step.capability, instruction: args.step.instruction, depends_on: args.step.dependsOn, status: args.status, attempts: args.attempts, result: args.result ?? null, error: args.error === undefined ? null : cleanError(args.error), started_at: args.status === "running" ? now : undefined, completed_at: ["completed", "failed", "blocked"].includes(args.status) ? now : undefined }; const { error } = await jobs(args.supabase).from("nuva_studio_job_steps").upsert(row, { onConflict: "job_id,step" }); if (error) throw new Error(error.message); }
export async function createStudioCallback(args: { supabase: SupabaseClient<Database>; jobId: string; step: number; callbackType: "video" | "media" | "generic"; tokenHash: string; expiresAt?: string }) { const { data, error } = await jobs(args.supabase).from("nuva_studio_job_callbacks").insert({ job_id: args.jobId, step: args.step, callback_type: args.callbackType, token_hash: args.tokenHash, expires_at: args.expiresAt ?? new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString() }).select("id").single(); if (error) throw new Error(error.message); return data as { id: string }; }
export async function getStudioJob(args: { supabase: SupabaseClient<Database>; jobId: string }) { const { data, error } = await jobs(args.supabase).from("nuva_studio_jobs").select("*").eq("id", args.jobId).maybeSingle(); if (error) throw new Error(error.message); return (data as StudioJobRow | null) ?? null; }
