...
export async function updateStudioJobCheckpoint(args: { supabase: SupabaseClient<Database>; jobId: string; checkpoint: StudioExecutionCheckpoint; status?: string; result?: StudioExecutionResult | null; lastError?: unknown; nextRunAt?: string | null }) {
  ...
  if (["completed", "partial", "blocked", "failed", "cancelled"].includes(args.status ?? "")) { patch.completed_at = new Date().toISOString(); patch.locked_at = null; }
  if (["queued", "waiting"].includes(args.status ?? "")) patch.locked_at = null;
...
export async function updateStudioJobStatus(args: { supabase: SupabaseClient<Database>; jobId: string; status: string; lastError?: unknown; nextRunAt?: string | null }) {
  const patch: Record<string, unknown> = { status: args.status, locked_at: null };
  if (args.lastError !== undefined) patch.last_error = args.lastError === null ? null : cleanError(args.lastError);
  if (args.nextRunAt !== undefined) patch.next_run_at = args.nextRunAt;
...
export async function incrementStudioJobAttempt(args: { supabase: SupabaseClient<Database>; jobId: string }) {
  ...
  if (["completed", "partial", "blocked", "cancelled"].includes(currentStatus)) return { allowed: false, attempts: Number(current.data.attempts ?? 0), busy: false };
  const lockedAt = current.data.locked_at ? new Date(current.data.locked_at).getTime() : 0;
  if (lockedAt && Date.now() - lockedAt < LOCK_TTL_MS) return { allowed: false, attempts: Number(current.data.attempts ?? 0), busy: true };
  const attempts = Number(current.data.attempts ?? 0) + 1;
  if (attempts > Number(current.data.max_attempts ?? 3)) return { allowed: false, attempts, busy: false };
  const staleBefore = new Date(Date.now() - LOCK_TTL_MS).toISOString();
  const claim = await client.from("nuva_studio_jobs").update({ attempts, status: "running", locked_at: new Date().toISOString(), next_run_at: null }).eq("id", args.jobId).or(`locked_at.is.null,locked_at.lt.${staleBefore}`).select("id").maybeSingle();
  if (claim.error) throw new Error(claim.error.message);
  if (!claim.data) return { allowed: false, attempts: attempts - 1, busy: true };
  return { allowed: true, attempts, busy: false };
}

export async function upsertStudioJobStep(args: { supabase: SupabaseClient<Database>; jobId: string; step: StudioExecutionStep; status: StudioStepStatus; attempts: number; result?: unknown; error?: unknown }) {
  const now = new Date().toISOString();
  const row = {
    job_id: args.jobId, step: args.step.index, capability: args.step.capability, instruction: args.step.instruction, depends_on: args.step.dependsOn,
    status: args.status, attempts: args.attempts, result: args.result ?? null, error: args.error === undefined ? null : cleanError(args.error),
    started_at: args.status === "running" ? now : undefined, completed_at: ["completed", "failed", "blocked"].includes(args.status) ? now : undefined,
  };
...
export async function createStudioCallback(args: { supabase: SupabaseClient<Database>; jobId: string; step: number; callbackType: "video" | "media" | "generic"; tokenHash: string; expiresAt?: string }) {
  const { data, error } = await jobs(args.supabase).from("nuva_studio_job_callbacks").insert({ job_id: args.jobId, step: args.step, callback_type: args.callbackType, token_hash: args.tokenHash, expires_at: args.expiresAt ?? new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString() }).select("id").single();
  if (error) throw new Error(error.message);
