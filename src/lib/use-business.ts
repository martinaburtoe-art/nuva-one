...
export function useActiveBusinessId() {
  ...
  const setId = useCallback((newId: string | null) => {
    if (typeof window !== "undefined") { if (newId) localStorage.setItem(KEY, newId); else localStorage.removeItem(KEY); }
    setIdState(newId);
  }, []);
...
export function canManageBusiness(role: MemberRole | null | undefined) { return role === "owner" || role === "admin"; }
export function isBusinessOwner(role: MemberRole | null | undefined) { return role === "owner"; }
...
export function defaultPermissionsForRole(role: MemberRole): ModulePermissions {
  const permissions = Object.fromEntries(MODULES.map((m) => [m.key, true])) as ModulePermissions;
  if (role === "viewer") permissions.people = false;
  return permissions;
}

export function hasModulePermission(role: MemberRole | null | undefined, permissions: ModulePermissions | null | undefined, moduleKey: ModuleKey) {
  if (role === "owner" || role === "admin") return true;
  if (!role) return false;
  if (moduleKey === "people" && role === "viewer") return false;
