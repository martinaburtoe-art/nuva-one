...
export async function runDueStudioCampaign(args: { supabase: SupabaseClient<Database>; campaign: Campaign }) {
  ...
  try {
    ...
    if (!plannedJob) {
      const planned = await createStudioPlanAndJob({ supabase: args.supabase, businessId: args.campaign.business_id, userId: args.campaign.user_id, prompt: objective, maxSteps: 6, idempotencyKey: `campaign:${args.campaign.id}:cycle:${cycleNumber}` });
      plannedJob = planned.job;
      await db(args.supabase).from("nuva_studio_campaign_cycles").update({ studio_job_id: plannedJob.id }).eq("id", cycle.id).is("studio_job_id", null);
    }
    ...
    await db(args.supabase).from("nuva_studio_campaign_cycles").update({ status: cycleStatus, completed_at: terminal ? new Date().toISOString() : null, learnings: { jobStatus: jobResult.status, completedSteps: jobResult.result?.completed?.length ?? 0, media: jobResult.result?.media ?? [] } }).eq("id", cycle.id);
    await db(args.supabase).from("nuva_studio_campaigns").update({ cycles_completed: terminal ? cycleNumber : args.campaign.cycles_completed, last_run_at: terminal ? new Date().toISOString() : args.campaign.last_run_at, next_run_at: terminal ? nextRunAt : new Date(Date.now() + 60 * 60 * 1000).toISOString(), updated_at: new Date().toISOString(), status: terminal && args.campaign.max_cycles > 0 && cycleNumber >= args.campaign.max_cycles ? "completed" : "active" }).eq("id", args.campaign.id).eq("cycles_completed", args.campaign.cycles_completed);
    return { skipped: false, campaignId: args.campaign.id, cycleId: cycle.id, jobId: plannedJob.id, status: jobResult.status };
  } catch (error) {
