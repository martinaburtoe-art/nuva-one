...
export function buildMorningBrief(input: MorningBriefInput): MorningBrief {
  ...
  const metrics = [
    ...
    ...(input.cashDays !== undefined
      ? [{ id: "cash-days", label: "Cobertura de caja", value: `${input.cashDays} días` }]
      : []),
    ...(input.overdueReceivables !== undefined
      ? [
          {
            id: "overdue",
            label: "Cobranza vencida",
            value: `$${input.overdueReceivables.toLocaleString("es-CL")}`,
          },
        ]
      : []),
    ...(input.lowStockSkus !== undefined
      ? [{ id: "stock", label: "SKU bajo reposición", value: `${input.lowStockSkus}` }]
      : []),
    ...(input.complianceReadiness !== undefined
      ? [{ id: "compliance", label: "Compliance", value: `${input.complianceReadiness}/100` }]
      : []),
  ];
