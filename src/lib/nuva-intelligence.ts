...
type Sale = { total?: number; sale_date?: string; status?: string; paid_amount?: number; due_date?: string };
type Purchase = { total?: number; purchase_date?: string; status?: string };
type Transaction = { amount?: number; type?: string; tx_date?: string };
type Stock = { quantity?: number; min_stock?: number; reorder_point?: number; name?: string; sku?: string };
...
export function buildNuvaIntelligence(input: {
  ...
  const expenses = transactions.filter((x) => x.type === "expense").reduce((s, x) => s + amount(x.amount), 0);
  const income = transactions.filter((x) => x.type === "income").reduce((s, x) => s + amount(x.amount), 0);

  const overdue = sales.filter((x) => {
    if (!x.due_date) return false;
    return new Date(x.due_date) < now && amount(x.paid_amount) < amount(x.total);
  });
  const overdueAmount = overdue.reduce((s, x) => s + Math.max(0, amount(x.total) - amount(x.paid_amount)), 0);
  ...
  if (overdueAmount > 0) {
    signals.push({
      ...
      action: "Prioriza cobranza y registra compromisos de pago antes de aumentar nuevas ventas a crédito.",
  ...
  if (revenue > 0 && purchaseSpend > revenue * 0.7) {
    ...
    signals.push({
      ...
      action: "Revisa margen por producto, rotación y condiciones de proveedores antes de aumentar inventario.",
      metric: ratio,
      unit: "percent",
  ...
  if (revenue > 0 && expenses < revenue * 0.15 && sales.length >= 3) {
    signals.push({
      ...
      explanation: "Las ventas muestran actividad mientras los gastos operativos registrados son relativamente bajos.",
      action: "Activa campañas de recompra y seguimiento de clientes antes de aumentar estructura de costos.",
      metric: revenue,
      unit: "clp",
  ...
  if (!signals.length) {
    signals.push({
      ...
      action: "Mantén el seguimiento y completa los datos de ventas, compras, caja e inventario para aumentar la precisión.",
      source: ["ventas", "compras", "caja", "inventario"],
    });
  }

  return signals.sort((a, b) => {
    const weight: Record<IntelligenceSeverity, number> = { critical: 4, warning: 3, opportunity: 2, info: 1 };
