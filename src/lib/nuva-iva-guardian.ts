...
export function buildIvaGuardian(input: IvaGuardianInput): IvaGuardianResult {
  ...
  const debitVat = round(
    internal.filter((d) => d.kind === "sale").reduce((sum, d) => sum + effectiveVat(d), 0),
  );
  const creditVat = round(
    internal.filter((d) => d.kind === "purchase").reduce((sum, d) => sum + effectiveVat(d), 0),
  );
  const adjustments = round(
    internal
      .filter((d) => d.kind === "credit_note" || d.kind === "debit_note")
      .reduce((sum, d) => sum + effectiveVat(d), 0),
  );
  ...
  if (unmatchedInternal.length > 0)
    alerts.push(
      "Existen documentos internos que no fueron encontrados en la fuente SII importada.",
    );
  if (unmatchedSii.length > 0)
    alerts.push("Existen documentos SII que no están conciliados con el registro interno.");
  if (Math.abs(grossDifference) > 1)
    alerts.push("Existe una diferencia de monto bruto entre registros internos y SII.");
  if (estimatedVat < 0)
    alerts.push(
      "El crédito/ajustes superan el débito estimado; revisar antes de preparar el período tributario.",
    );

  const discrepancyPenalty = Math.min(
    50,
    unmatchedInternal.length * 8 +
      unmatchedSii.length * 8 +
      (Math.abs(grossDifference) > 1 ? 20 : 0),
  );
