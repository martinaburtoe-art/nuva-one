...
export function assessChileTaxReadiness(input: TaxReadinessInput): TaxReadinessResult {
  ...
  const checks = [
    ...
    {
      id: "rcv-purchases",
      ok: input.rcvPurchaseUnreconciled === 0,
      label: "Compras RCV conciliadas",
    },
  ...
  if (input.dteRejected > 0)
    blockers.push(`${input.dteRejected} DTE rechazados requieren resolución.`);
  if (input.dtePending > 0)
    blockers.push(`${input.dtePending} DTE aún están pendientes de validación.`);
  if (input.dteMissingXml > 0)
    blockers.push(`${input.dteMissingXml} DTE no tienen XML disponible.`);
  if (input.rcvPurchaseUnreconciled > 0)
    blockers.push(`${input.rcvPurchaseUnreconciled} compras no están conciliadas con RCV.`);
  if (input.rcvSalesUnreconciled > 0)
    blockers.push(`${input.rcvSalesUnreconciled} ventas no están conciliadas con RCV.`);
  if (input.ivaDifference !== 0)
    blockers.push(`Existe una diferencia de IVA de ${input.ivaDifference}.`);
  if (input.f29Errors > 0)
    blockers.push(`${input.f29Errors} errores impiden considerar preparado el F29.`);
  if (!input.periodLocked)
    warnings.push(
      "El período aún no está bloqueado; se requiere revisión antes de una declaración final.",
    );
  if (input.f29Warnings > 0)
    warnings.push(`${input.f29Warnings} advertencias del F29 requieren revisión profesional.`);

  const acceptedRatio =
    input.dteAccepted + input.dteRejected + input.dtePending > 0
