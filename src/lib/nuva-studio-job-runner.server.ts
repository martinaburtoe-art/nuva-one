...
import { getRunnableSteps, markStep, resumeExecution, type StudioExecutionResult, type StudioMediaCapability } from "@/lib/nuva-studio-execution.server";
...
function mediaCapability(capability: AiCapability): StudioMediaCapability { if (capability === "image" || capability === "image_edit" || capability === "video" || capability === "voice") return capability; throw new Error(`Capacidad multimedia inválida: ${capability}`); }
  ...
  try {
    for (const step of getRunnableSteps(plan, checkpoint)) {
      ...
      if (MEDIA_CAPABILITIES.has(step.capability)) {
        const capability = mediaCapability(step.capability);
        const result = await withStudioJobLease({ supabase: args.supabase, jobId: args.jobId, lockToken, task: () => executeStudioMedia({ businessId: job.business_id, userId: args.userId, jobId: args.jobId, step: step.index, capability, prompt, supabase: args.supabase }) });
        ...
        if (result.status === "queued") { checkpoint = markStep(checkpoint, step.index, { status: "queued", attempts, result: JSON.stringify(result) }); await upsertStudioJobStep({ supabase: args.supabase, jobId: args.jobId, step, status: "queued", attempts, result }); await updateStudioJobCheckpoint({ supabase: args.supabase, jobId: args.jobId, checkpoint, status: "waiting", lockToken }); await recordStudioAudit({ supabase: args.supabase, businessId: job.business_id, userId: args.userId, jobId: job.id, action: "studio.job.waiting_callback", metadata: { step: step.index, callbackId: String(result.callbackId) } }); return await getStudioJob({ supabase: args.supabase, jobId: args.jobId }); }
        const errorMessage = typeof result.error === "string" ? result.error : "La ejecución multimedia falló.";
        checkpoint = markStep(checkpoint, step.index, { status: result.status, attempts, error: errorMessage }); await upsertStudioJobStep({ supabase: args.supabase, jobId: args.jobId, step, status: result.status, attempts, error: errorMessage, result }); await updateStudioJobCheckpoint({ supabase: args.supabase, jobId: args.jobId, checkpoint, status: result.status === "blocked" ? "blocked" : "failed", lastError: errorMessage, lockToken }); return await getStudioJob({ supabase: args.supabase, jobId: args.jobId });
      }
    ...
    const media = checkpoint.steps.filter((step) => ["queued", "blocked", "failed"].includes(step.status)).flatMap((step) => { if (!MEDIA_CAPABILITIES.has(step.capability)) return []; return [{ step: step.step, capability: mediaCapability(step.capability), status: step.status === "queued" ? "ready" as const : "blocked" as const, input: plan.find((item) => item.index === step.step)?.instruction ?? "", reason: step.error }]; });
    ...
    if (error instanceof Error && error.message === "Studio job lease lost; stale worker fenced") { await recordStudioAudit({ supabase: args.supabase, businessId: job.business_id, userId: args.userId, jobId: job.id, action: "studio.job.stale_worker_fenced", metadata: { attempt: claim.attempts } }); return await getStudioJob({ supabase: args.supabase, jobId: args.jobId }); }
    const nextAttempt = claim.attempts; const retryAt = new Date(Date.now() + exponentialBackoffMs(nextAttempt)).toISOString(); checkpoint = { ...checkpoint, status: "failed", updatedAt: new Date().toISOString() }; const nextStatus = nextAttempt < job.max_attempts ? "queued" : "dead_letter";
    await updateStudioJobCheckpoint({ supabase: args.supabase, jobId: args.jobId, checkpoint, status: nextStatus, lastError: error instanceof Error ? error.message : String(error), nextRunAt: nextStatus === "queued" ? retryAt : null, lockToken });
