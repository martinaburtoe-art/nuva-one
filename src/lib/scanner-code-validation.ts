...
export function classifyScannerCode(raw: string): NormalizedScannerCode {
  ...
  if (/^\d{13}$/.test(value))
    return { value, kind: "EAN-13", validChecksum: hasValidChecksum(value) };
  if (/^\d{8}$/.test(value))
    return { value, kind: "EAN-8", validChecksum: hasValidChecksum(value) };
  if (/^\d{12}$/.test(value))
    return { value, kind: "UPC-A", validChecksum: hasValidChecksum(value) };
  if (/^[A-Z0-9][A-Z0-9._/-]{1,63}$/.test(value))
    return { value, kind: "SKU", validChecksum: true };
...
export function validateScannedCode(raw: string): ScannerCodeValidation {
  ...
  if (code.kind === "UNKNOWN")
    return {
      kind: "invalid",
      value: code.value,
      message: "El código escaneado no tiene un formato compatible.",
    };
  if (!code.validChecksum)
    return {
      kind: "invalid",
      value: code.value,
      message: `El ${code.kind} tiene un dígito verificador inválido.`,
    };
