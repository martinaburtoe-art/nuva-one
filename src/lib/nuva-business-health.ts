export type BusinessHealthSale = { total?: number | string | null; sale_date?: string | null; status?: string | null };
export type BusinessHealthCustomer = { name?: string | null; status?: string | null };
export type BusinessHealthProduct = { name?: string | null; price?: number | string | null; cost?: number | string | null; stock?: number | string | null; low_stock_threshold?: number | string | null };
export type BusinessHealthActivity = { type?: string | null; completed?: boolean | null; due_date?: string | null };
export type BusinessHealthCashRow = { flow_date?: string | null; net_cash?: number | string | null };
...
const severityFor = (value: number, thresholds: [number, number, number, number]): SignalSeverity => {
...
export function buildBusinessHealthIntelligence({ sales, customers, products, activities, cashFlow, reconciliation, now = Date.now() }: BusinessHealthInput): BusinessHealthIntelligence {
  ...
  const momentum = previous30 > 0 ? boundedScore(50 + ((current30 - previous30) / previous30) * 50) : current30 > 0 ? 75 : 50;
  ...
  const customerCompleteness = customers.length
    ? customers.reduce((sum, customer) => sum + [customer.name, customer.status].filter(Boolean).length / 2, 0) / customers.length
    : 0;
  const productCompleteness = products.length
    ...
    : 0;
  const dataReadiness = boundedScore((customerCompleteness * 0.5 + productCompleteness * 0.5) * 100);
  ...
  const controls = reconciliation.length === 0 ? 50 : boundedScore(100 - Math.min(100, (reconciliationOpen / reconciliation.length) * 100));
  ...
  const availableDomains = [sales.length > 0, cashFlow.length > 0, products.length > 0, customers.length > 0, activities.length > 0, reconciliation.length > 0].filter(Boolean).length;
  ...
  if (salesDeltaPct !== null && salesDeltaPct <= -10) {
    signals.push({
      ...
      evidence: [{ metric: "current_30d", value: current30 }, { metric: "previous_30d", value: previous30 }, { metric: "variation_pct", value: Number(salesDeltaPct.toFixed(1)) }],
      action: "Revisar productos, clientes y canales que explican la caída antes de aplicar descuentos o aumentar gasto comercial.",
      expectedImpact: "Identificar el origen de la caída y priorizar una intervención comercial basada en evidencia.",
    });
    ...
    signals.push({
      ...
      why: "La comparación usa únicamente ventas con fecha válida y estados no cancelados ni borradores.",
      evidence: [{ metric: "current_30d", value: current30 }, { metric: "previous_30d", value: previous30 }, { metric: "variation_pct", value: Number(salesDeltaPct.toFixed(1)) }],
      action: "Revisar qué productos, clientes y canales están impulsando el crecimiento para replicar el patrón.",
  ...
  if (recentCash < 0) {
    signals.push({
      ...
      action: "Revisar vencimientos de cuentas por cobrar/pagar y gastos no esenciales antes de asumir nueva presión de caja.",
  ...
  if (criticalStock.length > 0) {
    signals.push({
      ...
      action: "Revisar demanda reciente y compras antes de reponer; priorizar productos con ventas activas y stock agotado.",
  ...
  if (overdueTasks > 0) {
    signals.push({
      ...
      evidence: [{ metric: "overdue_tasks", value: overdueTasks }, { metric: "open_tasks", value: openTasks }],
  ...
  if (reconciliationOpen > 0) {
    signals.push({
      ...
      evidence: [{ metric: "open_reconciliations", value: reconciliationOpen }, { metric: "total_reconciliations", value: reconciliation.length }],
      action: "Revisar y cerrar las conciliaciones pendientes antes de usar sus cifras como base de decisiones críticas.",
      expectedImpact: "Aumentar la confiabilidad de la información financiera utilizada por Nüva Intelligence.",
    });
  }
  ...
  if (dataReadiness < 60) {
    signals.push({
      ...
      evidence: [{ metric: "data_readiness", value: dataReadiness }, { metric: "confidence", value: confidence }],
      action: "Completar los datos mínimos de clientes y productos y conectar las fuentes disponibles antes de tomar decisiones basadas en tendencias.",
      expectedImpact: "Aumentar precisión, cobertura y confianza de los insights.",
    });
  }

  signals.sort((a, b) => ({ CRITICAL: 5, HIGH: 4, MEDIUM: 3, LOW: 2, INFO: 1 }[b.severity] - { CRITICAL: 5, HIGH: 4, MEDIUM: 3, LOW: 2, INFO: 1 }[a.severity]));

  return { health, healthMethod: "bottleneck", momentum, liquidity, dataReadiness, execution, controls, current30, previous30, recentCash, reconciliationOpen, confidence, signals };
}
