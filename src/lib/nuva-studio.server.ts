...
export async function runNuvaStudioTask(args: { businessId: string; capability: AiCapability; prompt: string; supabase: Parameters<typeof buildBusinessContext>[0] }) {
  ...
  const system = ["Eres Nüva Studio, el motor de creación y crecimiento de Nüva One.", CAPABILITY_INSTRUCTIONS[args.capability], "Usa el contexto empresarial disponible, pero nunca inventes datos que no aparezcan allí.", "Cuando falten datos críticos, dilo y propone una forma concreta de obtenerlos.", "Entrega resultados directamente utilizables por una MiPyme.", `CONTEXTO EMPRESARIAL:\n${contextText}`].join("\n\n");
...
export async function planNuvaStudioTask(args: { businessId: string; prompt: string; supabase: Parameters<typeof buildBusinessContext>[0] }) {
  const businessContext = await buildBusinessContext(args.supabase, args.businessId);
  const contextText = businessContext ? capContext(businessContext) : "No hay contexto empresarial disponible.";
  const system = ["Eres el planificador autónomo de Nüva One.", "Convierte objetivos empresariales complejos en un plan corto y ejecutable usando las capacidades de Nüva Studio.", "Usa el contexto empresarial y no inventes datos.", PLANNER_INSTRUCTION, `CONTEXTO EMPRESARIAL:\n${contextText}`].join("\n\n");
