...
export function buildNuvaActionCenter(signals: Signal[], limit = 5): NuvaAction[] {
  return signals.filter((signal) => Boolean(signal?.title)).map((signal, index) => {
    const destination = destinationMap[signal.id] ?? "dashboard";
    const severity = signal.severity ?? "info";
    return { id: signal.id || `signal-${index}`, priority: priorityMap[severity], title: signal.title, reason: signal.description, action: signal.action || signal.recommendation || "Revisar este indicador y definir una acción.", impact: impactMap[severity], destination, cta: ctaMap[destination], mode: modeMap[destination] };
  }).sort((a, b) => b.impact - a.impact).slice(0, Math.max(1, limit));
}
