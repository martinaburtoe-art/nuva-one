...
import type { StudioDatabase } from "@/lib/nuva-studio-db";
...
function asStudioClient(supabase: SupabaseClient): SupabaseClient<StudioDatabase> {
  return supabase as unknown as SupabaseClient<StudioDatabase>;
}
...
async function registerAsset(
  supabase: SupabaseClient<StudioDatabase>,
  args: {
    businessId: string;
    userId: string;
    jobId?: string;
...
export async function executeStudioMedia(args: {
  ...
  const supabase = asStudioClient(args.supabase);
  ...
  const quota = await supabase.rpc("reserve_ai_tool_quota", {
    p_business_id: args.businessId,
    p_tool_id: toolId,
    p_units: tool.cost_units,
  });
  if (quota.error || quota.data !== true) {
    return { step: args.step, capability: args.capability, status: "blocked", error: quota.error?.message ?? "Se alcanzó el límite de uso para esta capacidad." };
  }
  ...
  try {
    if (args.capability === "image" || args.capability === "image_edit") {
      ...
      await registerAsset(supabase, { ...args, storagePath: asset.storagePath, mimeType: asset.mimeType, model: asset.model });
    ...
    if (!webhook || !callbackBaseUrl || !args.jobId) {
      await supabase.rpc("release_ai_tool_quota", {
        p_business_id: args.businessId,
        p_tool_id: toolId,
        p_units: tool.cost_units,
      });
