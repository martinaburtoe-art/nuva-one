...
export type ExecutedMediaResult = { step: number; capability: MediaCapability; status: "completed" | "queued" | "blocked" | "failed"; storagePath?: string; signedUrl?: string; mimeType?: string; model?: string; error?: string };
...
async function registerAsset(supabase: SupabaseClient<Database>, args: { businessId: string; userId: string; step: number; capability: MediaCapability; storagePath?: string; mimeType?: string; model?: string; prompt: string }) {
  ...
  const { error } = await writer.from("ai_asset_library").insert({
    id: crypto.randomUUID(), business_id: args.businessId, user_id: args.userId,
...
export async function executeStudioMedia(args: { businessId: string; userId: string; step: number; capability: MediaCapability; prompt: string; supabase: SupabaseClient<Database> }): Promise<ExecutedMediaResult> {
  ...
  try {
    ...
    if (!webhook) {
      ...
      return { step: args.step, capability: args.capability, status: "blocked", error: "N8N_VIDEO_WEBHOOK_URL no está configurado; el trabajo de video quedó preparado sin consumir cuota." };
    }
    const response = await fetch(webhook, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ businessId: args.businessId, userId: args.userId, step: args.step, prompt: clean(args.prompt), provider: "google", model: "veo-3.1-generate-preview" }) });
