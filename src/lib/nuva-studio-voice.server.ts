...
function cleanText(value: string) {
  return value.replaceAll("\0", "").trim().slice(0, 10000);
}
...
export async function generateFishVoiceAsset(args: {
  ...
  const response = await fetch(FISH_TTS_URL, {
    ...
    body: JSON.stringify({ text: cleanText(args.prompt), format: "mp3", sample_rate: 44100, mp3_bitrate: 128, normalize: true, latency: "normal" }),
  });
  ...
  const { error: uploadError } = await args.supabase.storage.from("nuva-studio-assets").upload(storagePath, bytes, { contentType: "audio/mpeg", upsert: false });
  ...
  return { storagePath, signedUrl: signed.signedUrl, mimeType: "audio/mpeg", model: FISH_MODEL };
}
