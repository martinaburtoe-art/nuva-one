...
export type ProductResolverStatus = 'FOUND' | 'NOT_FOUND' | 'DUPLICATE' | 'INVALID' | 'UNAUTHORIZED';
...
export async function resolveProductCode(value: string): Promise<ProductResolution> {
  ...
  if (error) {
    ...
    if (error.code === '42501' || message.includes('permission') || message.includes('not authorized')) {
