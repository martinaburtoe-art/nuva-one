import { buildNuvaIntelligence, type IntelligenceSignal, type IntelligenceSeverity } from "./nuva-intelligence";
...
export function buildNuvaDecision(input: Parameters<typeof buildNuvaIntelligence>[0], actionLimit = 5): NuvaDecision {
  ...
  const status = critical ? "critical" : warning ? "attention" : opportunity ? "opportunity" : "stable";
  const headline = critical ? "Hay decisiones críticas que requieren atención hoy." : warning ? "Hay puntos de atención que conviene resolver hoy." : opportunity ? "Nüva detectó oportunidades para mejorar el negocio." : "El negocio se encuentra estable con los datos disponibles.";
  return { score, status, headline, topSignal, actions: buildNuvaActionCenter(signals, actionLimit), signals };
}
