...
export async function uploadBusinessPhoto(businessId: string, file: File): Promise<string> {
  ...
  const { error } = await supabase.storage.from("business-showcase").upload(path, file, {
    cacheControl: "3600",
    upsert: false,
  });
  if (error) throw error;

  const { data } = supabase.storage.from("business-showcase").getPublicUrl(path);
  return data.publicUrl;
}

export async function deleteBusinessPhoto(url: string): Promise<void> {
  const marker = "/business-showcase/";
  const idx = url.indexOf(marker);
  if (idx === -1) return;
  const path = url.slice(idx + marker.length);
  await supabase.storage.from("business-showcase").remove([path]);
}
