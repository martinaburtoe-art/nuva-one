...
export const CHILE_COMPLIANCE_RULES: ComplianceRule[] = [
  {
    ...
    summary:
      "La Resolución Exenta SII N°207/2025 elimina la obligación de imprimir el timbre electrónico en la representación impresa de la boleta electrónica.",
  ...
  },
  {
    ...
    summary:
      "Desde el 26 de abril de 2026 el límite ordinario semanal baja a 42 horas; la reducción final a 40 horas opera el 26 de abril de 2028.",
    ...
    action:
      "Revisar jornadas, contratos, turnos y reglas de asistencia para evitar superar el límite aplicable.",
  },
  {
    ...
    summary:
      "La Ley N°21.719 regula el tratamiento y protección de datos personales y crea una Agencia de Protección de Datos Personales; entra en vigencia el 1 de diciembre de 2026.",
    ...
    action:
      "Preparar inventario de tratamientos, derechos de titulares, retención, contratos y controles de privacidad.",
  },
  {
    ...
    summary:
      "La gradualidad de la Ley N°21.561 culmina el 26 de abril de 2028 con un límite de 40 horas semanales.",
    effectiveOn: "2028-04-26",
    status: "upcoming",
    source: "https://www.dt.gob.cl/portal/1626/w3-propertyname-2556.html",
    sourceLabel: "Dirección del Trabajo — Ley 40 horas",
    action:
      "Planificar turnos y capacidad operativa para la siguiente reducción sin afectar continuidad del negocio.",
  },
];
...
export function getComplianceStatus(rule: ComplianceRule, now = new Date()): ComplianceStatus {
  return startOfDay(new Date(rule.effectiveOn + "T00:00:00")) <= startOfDay(now)
    ? "active"
    : rule.status;
}

export function getComplianceRadar(now = new Date()) {
  return CHILE_COMPLIANCE_RULES.map((rule) => ({
    ...rule,
    status: getComplianceStatus(rule, now),
