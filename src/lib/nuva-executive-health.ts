...
function metricScore(metric: ExecutiveMetric): number {
  if (!Number.isFinite(metric.value) || !Number.isFinite(metric.target) || metric.target === 0) return 0;
  const ratio = metric.higherIsBetter === false ? metric.target / Math.max(metric.value, 0.000001) : metric.value / metric.target;
...
export function buildExecutiveHealth(metrics: ExecutiveMetric[]): ExecutiveHealth {
  ...
  const score = totalWeight === 0 ? 100 : Math.round(scored.reduce((sum, metric) => sum + metric.score * Math.max(metric.weight, 0), 0) / totalWeight);
  const weakestMetric = scored.length ? [...scored].sort((a, b) => a.score - b.score)[0].label : null;
