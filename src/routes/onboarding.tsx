    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-mesh px-4 py-8 sm:px-6 sm:py-12">
      <Card className="w-full max-w-2xl overflow-hidden shadow-elegant">
        ...
        <div className="px-6 py-8 sm:px-8 sm:py-10">
          ...
          {step === 2 && <div className="animate-fade-in-up"><span className="text-xs font-semibold uppercase tracking-wider text-primary">Paso 2 de 4</span><h1 className="mt-2 text-2xl font-bold sm:text-3xl">Personalicemos tu experiencia.</h1><p className="mt-2 max-w-lg text-sm text-muted-foreground">Nüva One utilizará esta información para darte un contexto más útil y adaptar sus recomendaciones.</p><div className="mt-7 grid gap-5 sm:grid-cols-2"><div className="space-y-3"><Label>Industria</Label><Select value={industry} onValueChange={setIndustry}><SelectTrigger className="h-11"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="retail">Retail / Comercio</SelectItem><SelectItem value="food">Gastronomía</SelectItem><SelectItem value="services">Servicios profesionales</SelectItem><SelectItem value="manufacturing">Manufactura</SelectItem><SelectItem value="health">Salud</SelectItem><SelectItem value="construction">Construcción</SelectItem><SelectItem value="other">Otro</SelectItem></SelectContent></Select></div><div className="space-y-3"><Label>Tamaño del equipo</Label><Select value={size} onValueChange={setSize}><SelectTrigger className="h-11"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="solo">Solo yo</SelectItem><SelectItem value="1-5">1–5 personas</SelectItem><SelectItem value="6-10">6–10 personas</SelectItem><SelectItem value="11-25">11–25 personas</SelectItem><SelectItem value="26-50">26–50 personas</SelectItem><SelectItem value="51+">51+ personas</SelectItem></SelectContent></Select></div></div><div className="mt-8 flex justify-between"><Button variant="ghost" onClick={() => setStep(1)}><ArrowLeft className="mr-1.5 h-4 w-4" />Atrás</Button><Button onClick={() => setStep(3)} size="lg">Continuar <ArrowRight className="ml-1.5 h-4 w-4" /></Button></div></div>}
