...
function PeoplePayroll() {
  ...
  async function createPeriod() {
    const created = await insertPeriod.mutateAsync({ period_year: year, period_month: month, status: "draft", calculation_version: "cl-2026.3" });
    setSelectedPeriod(created.id);
  }

  async function saveInput() {
    if (!selectedPeriod || !selectedEmployee) return;
    await insertInput({ payroll_period_id: selectedPeriod, employee_id: selectedEmployee, ...Object.fromEntries(Object.entries(input).map(([k, v]) => [k, typeof v === "number" ? Number(v) || 0 : v])) });
    setInput(emptyInput);
  }
  ...
  async function periodAction(periodId: string, action: "calculate" | "approve" | "lre" | "liquidate" | "finance" | "close") {
    ...
    try {
      const rpc = { calculate: "calculate_people_payroll_period", approve: "approve_people_payroll_period", lre: "prepare_people_lre", liquidate: "generate_people_liquidations", finance: "post_people_payroll_to_finance", close: "close_people_payroll_period" }[action];
      const { data, error } = await supabase.rpc(rpc as any, { p_payroll_period_id: periodId });
      if (error) throw error;
      const result = data as any;
      setEngineMessage(action === "calculate" ? `Nómina calculada: ${result?.items ?? 0} colaboradores.` : action === "lre" ? `LRE preparado: ${result?.valid ?? 0} válidos / ${result?.invalid ?? 0} con observaciones.` : action === "liquidate" ? `Liquidaciones generadas: ${result?.liquidations ?? 0}.` : action === "finance" ? `Nómina enviada a revisión financiera: ${result?.employer_cost ?? 0} CLP.` : action === "approve" ? "Período aprobado. El cálculo queda protegido para el cierre." : action === "close" ? "Período cerrado y validado contra LRE." : "Operación completada.");
    } catch (error) {
  ...
  return <ModuleGuard module="people"><div className="p-4 md:p-6">
    ...
    <Card className="mt-5 rounded-2xl p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">2. Datos variables del mes</h2><p className="text-sm text-muted-foreground">Horas extra, bonos, ausencias, anticipos, descuentos y gratificación.</p></div><span className="text-xs text-muted-foreground">{selectedInputs.length} entradas</span></div>
      {!selectedPeriod ? <p className="mt-4 rounded-xl border border-dashed p-4 text-sm text-muted-foreground">Crea o selecciona un período para cargar datos.</p> :
      <div className="mt-4 grid gap-3 lg:grid-cols-4">
        ...
        <div className="lg:col-span-4"><Label>Notas</Label><Input value={input.notes} onChange={e=>setInput(v=>({...v,notes:e.target.value}))} placeholder="Observación opcional" /></div>
        <div className="lg:col-span-4 flex justify-end"><Button onClick={saveInput} disabled={insertInput.isPending || !selectedEmployee}><Plus className="mr-2 h-4 w-4"/>Guardar datos del mes</Button></div>
      </div>}
    ...
    <Card className="mt-5 rounded-2xl p-5"><h2 className="font-semibold">3. Períodos y acciones</h2><div className="mt-4 space-y-3">{periods.length === 0 ? <p className="text-sm text-muted-foreground">No hay períodos creados.</p> : periods.map((p:any)=>{const locked=["closed","void"].includes(p.status); return <div key={p.id} className={`rounded-xl border p-3 ${selectedPeriod===p.id?"border-indigo-500/50 bg-indigo-500/5":""}`} onClick={()=>setSelectedPeriod(p.id)}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium">{p.period_month}/{p.period_year}</p><p className="text-xs text-muted-foreground">Estado: {p.status} · {items.filter((x:any)=>x.payroll_period_id===p.id).length} calculados</p></div><div className="flex flex-wrap gap-2">{p.status==="draft"&&<Button size="sm" variant="outline" disabled={busyId===p.id+"calculate"} onClick={(e)=>{e.stopPropagation();periodAction(p.id,"calculate")}}><Calculator className="mr-1 h-3.5 w-3.5"/>Calcular</Button>}{p.status==="calculated"&&<Button size="sm" variant="outline" disabled={busyId===p.id+"approve"} onClick={(e)=>{e.stopPropagation();periodAction(p.id,"approve")}}><FileCheck2 className="mr-1 h-3.5 w-3.5"/>Aprobar</Button>}{["calculated","approved"].includes(p.status)&&<Button size="sm" variant="outline" disabled={busyId===p.id+"lre"} onClick={(e)=>{e.stopPropagation();periodAction(p.id,"lre")}}><FileText className="mr-1 h-3.5 w-3.5"/>LRE</Button>}{["approved","closed"].includes(p.status)&&<Button size="sm" variant="outline" disabled={busyId===p.id+"liquidate"} onClick={(e)=>{e.stopPropagation();periodAction(p.id,"liquidate")}}>Liquidaciones</Button>}{["approved","closed"].includes(p.status)&&<Button size="sm" variant="outline" disabled={busyId===p.id+"finance"} onClick={(e)=>{e.stopPropagation();periodAction(p.id,"finance")}}>Enviar a Finanzas</Button>}{p.status==="approved"&&!locked&&<Button size="sm" disabled={busyId===p.id+"close"} onClick={(e)=>{e.stopPropagation();periodAction(p.id,"close")}>Cerrar</Button>}</div></div></div>})}</div></Card>
