...
function PeoplePayroll() {
  ...
  const latestParams = useMemo(() => params.slice(0, 8), [params]);

  async function createPeriod() {
    await insertPeriod.mutateAsync({ period_year: year, period_month: month, status: "draft", calculation_version: "cl-2026.2" });
  }

  async function calculatePeriod(periodId: string) {
    setCalculatingId(periodId);
    setEngineMessage(null);
    try {
      const { data, error } = await supabase.rpc("calculate_people_payroll_period" as any, { p_payroll_period_id: periodId });
      if (error) throw error;
      setEngineMessage(`Nómina calculada: ${(data as any)?.items ?? 0} colaboradores procesados. El período queda en estado calculado.`);
    } catch (error) {
      setEngineMessage(error instanceof Error ? error.message : "No fue posible calcular el período.");
    } finally {
  ...
  return <ModuleGuard module="people"><div className="p-4 md:p-6">
    ...
    <Card className="mb-5 rounded-2xl border-amber-500/30 bg-amber-500/5 p-4"><div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5" /><div><p className="font-semibold">Motor protegido por parámetros</p><p className="text-sm text-muted-foreground">Las reglas legales se almacenan con vigencia y fuente. Nüva calcula un borrador auditable, pero no permite tratarlo como nómina productiva hasta completar las validaciones pendientes.</p></div></div></Card>
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      <Card className="rounded-2xl p-5"><LockKeyhole className="h-5 w-5" /><p className="mt-3 font-semibold">Cierre</p><p className="mt-1 text-sm text-muted-foreground">Aprobación y cierre quedan restringidos a owner/admin y el cálculo conserva un snapshot de parámetros.</p></Card>
    </div>
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-2">
      <Card className="rounded-2xl p-5"><h2 className="font-semibold">Períodos</h2><div className="mt-4 space-y-2">{periods.length === 0 ? <p className="text-sm text-muted-foreground">No hay períodos creados.</p> : periods.map((p: any) => <div key={p.id} className="flex items-center justify-between gap-3 rounded-xl border p-3 text-sm"><div><p className="font-medium">{p.period_month}/{p.period_year}</p><p className="text-xs text-muted-foreground">{p.status}</p></div><Button size="sm" variant="outline" disabled={calculatingId === p.id || ["approved","closed","void"].includes(p.status)} onClick={() => calculatePeriod(p.id)}>{calculatingId === p.id ? "Calculando…" : "Calcular"}</Button></div>)}</div></Card>
      <Card className="rounded-2xl p-5"><h2 className="font-semibold">Parámetros legales cargados</h2><div className="mt-4 space-y-2">{latestParams.map((p: any) => <div key={p.id} className="rounded-xl border p-3"><div className="flex justify-between gap-3 text-sm"><span>{p.parameter_key}</span><span>{p.value_numeric}</span></div><p className="mt-1 text-xs text-muted-foreground">Vigente desde {p.effective_from}</p></div>)}</div></Card>
    </div>
  </div></ModuleGuard>;
}
