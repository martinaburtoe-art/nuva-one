...
function StudioPage() {
  ...
  async function run() {
    ...
    try {
      ...
      setResult({ text: data.result?.text ?? "Tarea completada.", imageUrl: data.result?.imageUrl, audioUrl: data.result?.audioUrl, mediaUrl: data.result?.mediaUrl });
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border bg-gradient-to-br from-background via-background to-primary/5 p-5 md:p-7">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {actions.map((action) => {
            ...
            return (
              <button key={action.id} type="button" onClick={() => { setCapability(action.id); setPrompt(action.prompt); }} className={cn("rounded-xl border p-4 text-left transition-all hover:-translate-y-0.5 hover:border-primary", isSelected && "border-primary bg-primary/5 shadow-sm")}>
      ...
      {result && (
        <section className="rounded-2xl border bg-card p-5 md:p-7">
          ...
          {result.imageUrl && <div className="mt-5 overflow-hidden rounded-xl border bg-muted"><img src={result.imageUrl} alt="Activo generado por Nüva Studio" className="mx-auto max-h-[640px] w-full object-contain" /></div>}
          {result.audioUrl && <div className="mt-5 rounded-xl border bg-muted p-4"><audio controls className="w-full" src={result.audioUrl}>Tu navegador no puede reproducir este audio.</audio></div>}
          {result.mediaUrl && <div className="mt-5 overflow-hidden rounded-xl border bg-black"><video controls className="mx-auto max-h-[640px] w-full" src={result.mediaUrl}>Tu navegador no puede reproducir este video.</video></div>}
        </section>
      )}
      <section className="grid gap-4 md:grid-cols-3">
        {[
          ...
          ["Biblioteca de activos", "Las imágenes, locuciones y videos se guardan como activos reutilizables del negocio."],
        ].map(([title, description]) => <div key={title} className="rounded-xl border bg-card p-5"><p className="text-sm font-semibold">{title}</p><p className="mt-2 text-xs leading-5 text-muted-foreground">{description}</p></div>)}
      </section>
    </div>
  );
}
