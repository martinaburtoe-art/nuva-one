...
function CashControl() {
  ...
  const registerQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id, opening_amount, counted_cash, status, opened_at")
  ...
  const todaySales = useMemo(
    () => sales.filter((s: any) => (s.sale_date ?? s.created_at?.slice(0, 10)) === today && s.status === "paid"),
    [sales, today],
  );
  ...
  const deposits = (movementsQuery.data ?? [])
    .filter((m) => m.movement_type === "deposit")
    .reduce((sum, m) => sum + Number(m.amount), 0);
  const withdrawals = (movementsQuery.data ?? [])
    .filter((m) => m.movement_type === "withdrawal")
    .reduce((sum, m) => sum + Number(m.amount), 0);
  const expectedCash = registerQuery.data
    ? Number(registerQuery.data.opening_amount) + cashSales + deposits - withdrawals
    : 0;
  ...
  async function openRegister() {
    ...
    try {
      ...
      const { error } = await supabase.from("cash_registers" as any).insert({
        ...
        opened_by: session.session?.user.id,
      ...
      toast.error(error?.message?.includes("uq_cash_register") ? "Ya existe una caja abierta para este negocio" : "No se pudo abrir la caja");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function addMovement() {
    if (!active || !registerQuery.data || !movementDialog || !canWrite) return;
    const amount = Number(movementAmount);
    if (!Number.isFinite(amount) || amount <= 0 || !movementReason.trim()) {
      return toast.error("Ingresa monto y motivo");
    }
    setBusy(true);
    try {
      const { data: session } = await supabase.auth.getSession();
      const { error } = await supabase.from("cash_register_movements" as any).insert({
        business_id: active.id,
        cash_register_id: registerQuery.data.id,
  ...
  async function closeRegister() {
    if (!active || !registerQuery.data || !canWrite) return;
    ...
    try {
      ...
      const { error } = await supabase
        .from("cash_registers" as any)
        .update({ status: "closed", closed_by: session.session?.user.id, closed_at: new Date().toISOString(), counted_cash: counted })
        .eq("id", registerQuery.data.id)
        .eq("business_id", active.id)
        .eq("status", "open");
      if (error) throw error;
    ...
    <ModuleGuard module="pos">
      <div className="space-y-6">
        <PageHeader
          ...
          description="Apertura, control de efectivo, movimientos y arqueo"
          action={
            <div className="flex items-center gap-2">
              <Badge variant={registerQuery.data ? "default" : "secondary"}>
                {registerQuery.data ? "Caja abierta" : "Caja cerrada"}
              </Badge>
              <Button variant="ghost" size="icon" onClick={() => queryClient.invalidateQueries()} aria-label="Actualizar">
        ...
        {loading ? (
          ...
          <Card className="border-dashed p-8 text-center">
            ...
            <Button className="mt-5" disabled={!canWrite} onClick={() => setOpenDialog(true)}>
          ...
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <Metric title="Fondo inicial" value={fmtCLP(Number(registerQuery.data.opening_amount))} icon={<WalletCards className="h-4 w-4" />} />
              <Metric title="Ventas efectivo" value={fmtCLP(cashSales)} icon={<Banknote className="h-4 w-4" />} />
              <Metric title="Efectivo esperado" value={fmtCLP(expectedCash)} icon={<ArrowDownToLine className="h-4 w-4" />} />
              <Metric title="Ventas no efectivo" value={fmtCLP(cardSales + transferSales)} icon={<WalletCards className="h-4 w-4" />} />
            </div>
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_380px]">
              <Card className="p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <h2 className="text-lg font-semibold">Control de jornada</h2>
                    <p className="text-sm text-muted-foreground">Ventas y movimientos que afectan directamente al efectivo.</p>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="outline" disabled={!canWrite} onClick={() => setMovementDialog("deposit")}>
                      <ArrowDownToLine className="mr-2 h-4 w-4" /> Ingreso
                    </Button>
                    <Button variant="outline" disabled={!canWrite} onClick={() => setMovementDialog("withdrawal")}>
                      <ArrowUpFromLine className="mr-2 h-4 w-4" /> Retiro
                    </Button>
                  </div>
                </div>
                <div className="mt-5 grid gap-3 sm:grid-cols-3">
                  <Summary label="Ingresos manuales" value={fmtCLP(deposits)} />
                  <Summary label="Retiros" value={fmtCLP(withdrawals)} />
                  <Summary label="Ventas" value={String(todaySales.length)} />
                </div>
                  ...
                  {(movementsQuery.data ?? []).slice(0, 8).map((m) => (
                    <div key={m.id} className="flex items-center justify-between rounded-lg border p-3 text-sm">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{new Date(m.created_at).toLocaleTimeString("es-CL", { hour: "2-digit", minute: "2-digit" })}</div>
                      </div>
                      <span className={m.movement_type === "deposit" ? "font-semibold text-emerald-600" : "font-semibold text-destructive"}>
                        {m.movement_type === "deposit" ? "+" : "-"}{fmtCLP(Number(m.amount))}
                      </span>
                    </div>
                  ))}
              ...
              <Card className="p-5">
                ...
                <p className="text-sm text-muted-foreground">Compara el efectivo contado con el esperado.</p>
                <div className="mt-5 rounded-xl border p-4">
                  <div className="flex justify-between text-sm"><span>Esperado</span><strong>{fmtCLP(expectedCash)}</strong></div>
                  <div className="mt-2 flex justify-between text-sm"><span>Contado</span><strong>{countedCash === "" ? "—" : fmtCLP(Number(countedCash))}</strong></div>
                  <div className="mt-3 border-t pt-3 flex justify-between"><span className="font-medium">Diferencia</span><strong className={difference == null ? "" : difference === 0 ? "text-emerald-600" : "text-destructive"}>{difference == null ? "—" : fmtCLP(difference)}</strong></div>
                </div>
                <Button className="mt-4 w-full" disabled={!canWrite} onClick={() => setCloseDialog(true)}>
                  <LockKeyhole className="mr-2 h-4 w-4" /> Cerrar y arquear caja
                </Button>
              </Card>
            </div>
          </>
...
function Metric({ title, value, icon }: { title: string; value: string; icon: React.ReactNode }) {
  return <Card className="p-4"><div className="flex items-center gap-2 text-xs text-muted-foreground">{icon}{title}</div><div className="mt-2 text-xl font-bold">{value}</div></Card>;
}

function Summary({ label, value }: { label: string; value: string }) {
  return <div className="rounded-lg bg-muted/40 p-3"><div className="text-xs text-muted-foreground">{label}</div><div className="mt-1 font-semibold">{value}</div></div>;
}
