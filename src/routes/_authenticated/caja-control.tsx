...
function CashControl() {
  ...
  const registerQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id,opening_amount,counted_cash,status,opened_at,closed_at")
  ...
  const summaryQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      return (Array.isArray(data) ? data[0] : data) as Summary;
    },
  });
  ...
  async function openRegister() {
    ...
    if (!Number.isFinite(amount) || amount < 0)
      return toast.error("Ingresa un fondo inicial válido");
    setBusy(true);
    try {
      ...
      toast.error(
        (e?.message ?? "").includes("CASH_REGISTER_ALREADY_OPEN")
  ...
  async function addMovement() {
    ...
    try {
      ...
      setMovementAmount("");
      setMovementReason("");
      await qc.invalidateQueries({ queryKey: ["cash-register-movements", registerQuery.data.id] });
      await qc.invalidateQueries({ queryKey: ["cash-register-summary", registerQuery.data.id] });
    } catch (e: any) {
      toast.error(
        (e?.message ?? "").includes("INSUFFICIENT_EXPECTED_CASH")
      ...
      <div className="space-y-6">
        ...
        {registerQuery.isLoading ? (
          ...
          <Card className="border-dashed p-8 text-center">
            ...
            <p className="mx-auto mt-1 max-w-md text-sm text-muted-foreground">
              Define el fondo inicial para comenzar a controlar el efectivo de esta jornada.
            </p>
          ...
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              <Metric
                title="Fondo inicial"
                value={fmtCLP(Number(s?.opening_amount ?? registerQuery.data.opening_amount))}
              ...
              <Metric
                title="Tarjetas"
                value={fmtCLP(Number(s?.card_sales ?? 0))}
                icon={<CreditCard className="h-4 w-4" />}
              />
              <Metric
                title="Transferencias"
                value={fmtCLP(Number(s?.transfer_sales ?? 0))}
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_380px]">
              <Card className="p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      La jornada se calcula desde la apertura real de caja, no solo desde el día
                      calendario.
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      disabled={!canWrite}
                      onClick={() => setMovementDialog("deposit")}
                    >
                    ...
                    <Button
                      variant="outline"
                      disabled={!canWrite}
                      onClick={() => setMovementDialog("withdrawal")}
                    >
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  <Summary label="Ventas registradas" value={String(s?.sales_count ?? 0)} />
                  <Summary label="Ingresos manuales" value={fmtCLP(Number(s?.deposits ?? 0))} />
                  <Summary label="Retiros" value={fmtCLP(Number(s?.withdrawals ?? 0))} />
                  <Summary label="Otros medios" value={fmtCLP(Number(s?.other_sales ?? 0))} />
                </div>
                <div className="mt-5 rounded-xl border p-4">
                  <div className="text-sm font-medium">Desglose de recaudación</div>
                  <div className="mt-3 space-y-2 text-sm">
                    <Row label="Efectivo" value={fmtCLP(Number(s?.cash_sales ?? 0))} />
                    <Row label="Tarjetas" value={fmtCLP(Number(s?.card_sales ?? 0))} />
                    <Row label="Transferencias" value={fmtCLP(Number(s?.transfer_sales ?? 0))} />
                    <Row label="Otros" value={fmtCLP(Number(s?.other_sales ?? 0))} />
              ...
              <Card className="p-5">
                ...
                <p className="text-sm text-muted-foreground">
                  Compara el efectivo físico con el esperado por el sistema.
                </p>
                <div className="mt-5 rounded-xl border p-4">
                  ...
                  <Row label="Ingresos" value={fmtCLP(Number(s?.deposits ?? 0))} />
                  <Row label="Retiros" value={fmtCLP(Number(s?.withdrawals ?? 0))} />
                  ...
                  <div className="mt-2">
                    ...
                    <Row
                      label="Diferencia"
                      value={difference === null ? "—" : fmtCLP(difference)}
                      strong
                    />
                  </div>
                </div>
        ...
        <Card className="p-5">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Consulta las últimas jornadas y sus arqueos.
              </p>
            </div>
          </div>
              ...
              <tbody>
                {(historyQuery.data ?? []).map((r) => (
                  <tr key={r.id} className="border-t">
                    <td className="px-3 py-3">{new Date(r.opened_at).toLocaleString("es-CL")}</td>
                    <td className="px-3 py-3">
                      {r.closed_at ? new Date(r.closed_at).toLocaleString("es-CL") : "—"}
                    </td>
                    <td className="px-3 py-3 text-right">{fmtCLP(Number(r.opening_amount))}</td>
                    <td className="px-3 py-3 text-right">
                      {r.counted_cash == null ? "—" : fmtCLP(Number(r.counted_cash))}
                    </td>
                    <td className="px-3 py-3">
        ...
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              />
              <Button className="w-full" disabled={busy} onClick={openRegister}>
                Confirmar apertura
              </Button>
            </div>
          </DialogContent>
        ...
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {movementDialog === "deposit" ? "Registrar ingreso" : "Registrar retiro"}
              </DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div>
              ...
              <Button className="w-full" disabled={busy} onClick={addMovement}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div className="rounded-lg border p-3 text-sm">
                Efectivo esperado: <strong>{fmtCLP(expected)}</strong>
              </div>
              <div>
              ...
              {counted !== null && (
                <div
                  className={
                    difference === 0
                      ? "rounded-lg border p-3 text-sm text-emerald-600"
                      : "rounded-lg border p-3 text-sm text-destructive"
                  }
                >
                  Diferencia: <strong>{fmtCLP(difference ?? 0)}</strong>
                </div>
              )}
              <Button className="w-full" disabled={busy} onClick={closeRegister}>
                Confirmar cierre
              </Button>
            </div>
          </DialogContent>
