...
function POS() {
    ...
    setCart((current) => {
      ...
      if (existing) {
        if (existing.qty + 1 > stock) {
          toast.error(`Solo quedan ${stock} de ${p.name}`);
          return current;
        }
      ...
      return [
        ...current,
        { product_id: p.id, name: p.name, qty: 1, price: Number(p.price ?? 0), stock },
      ];
    });
  }
  ...
  async function handleLiveDetected(result: { rawValue: string; format?: string }) {
    ...
    try {
      ...
      if (resolution.status === "NOT_FOUND") {
        setScannerResolution(null);
        toast.error(`Código ${code} no registrado`);
        setQuery(code);
        return;
      }
      if (resolution.status === "DUPLICATE")
        return toast.error("Este código tiene múltiples productos asociados");
      if (resolution.status === "UNAUTHORIZED")
        return toast.error("No autorizado para consultar este producto");
      toast.error("Código inválido");
    } catch (error: any) {
      toast.error(error?.message ?? "No se pudo resolver el código");
    }
  }
  function openLiveScanner(mode: "cart" | "lookup" = "cart") {
    if (!canWrite) return;
    setScannerMode(mode);
    setScannerResolution(null);
    setScannerOpen(true);
  ...
  async function checkout() {
      ...
      try {
        ...
        const res = await fetch("/api/billing/payments/create", {
          ...
          body: JSON.stringify({
            ...
            sale_id: saved?.id,
            amount: total,
            subject: `Venta ${matchedCustomer?.name ?? "mostrador"}`,
          }),
        });
    ...
    try {
      ...
      /* hook handles persistence errors */
    }
  }
  ...
  return (
    <ModuleGuard module="pos">
      <>
            ...
            <div className="flex items-center gap-2">
              ...
              {active?.id && (
                <MobileScannerBridge
                  ...
                  onScan={(code) =>
                    void handleLiveDetected({ rawValue: code, format: "mobile-camera" })
                  }
                />
              )}
            ...
            <div className="mt-3 space-y-3 border-t pt-3">
                  ...
                  <>
                    ...
                    {rutLooksValid && !foundByRut && (
                      <div className="mt-2 space-y-2 rounded-lg border border-dashed p-2.5">
                        <p className="text-xs text-muted-foreground">
                          No hay un cliente con ese RUT. Regístralo:
                        </p>
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {(["efectivo", "tarjeta", "transferencia", "online"] as PayMethod[]).map((m) => (
                    <button
                      key={m}
                      onClick={() => (m !== "online" || hasPaymentGateway) && setMethod(m)}
                      disabled={m === "online" && !hasPaymentGateway}
                      className={cn(
                        "rounded-lg border px-2 py-2 text-xs font-medium capitalize transition",
                        method === m
                          ? "border-primary bg-primary text-primary-foreground"
                          : m === "online" && !hasPaymentGateway
              ...
              <Button
                ...
                onClick={checkout}
                disabled={!canPay || insert.isPending || creatingLink || !canWrite}
              >
                {!canWrite
                  ? "Solo lectura — no puedes cobrar"
                  : insert.isPending || creatingLink
                    ? "Procesando…"
                    : method === "online"
                      ? `Generar link de pago · ${fmtCLP(total)}`
                      : `Cobrar ${fmtCLP(total)}`}
              </Button>
            </div>
          </Card>
        ...
        <LiveScannerView
          open={scannerOpen}
          title={scannerMode === "cart" ? "Escanear en Caja" : "Buscar producto"}
          onDetect={handleLiveDetected}
          onClose={() => setScannerOpen(false)}
          onError={(error) => console.error("Nüva Live scanner", error)}
        />
        {scannerResolution && scannerMode === "lookup" && (
          <Card className="mt-4 border-primary/20 bg-primary/5 p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs text-muted-foreground">Producto identificado</div>
                <div className="font-semibold">{scannerResolution.name}</div>
                <div className="text-xs text-muted-foreground">
                  SKU: {scannerResolution.sku ?? "—"} · Stock: {scannerResolution.stock ?? 0}
                </div>
              </div>
              <Button onClick={() => addResolvedProduct(scannerResolution)}>Agregar a Caja</Button>
            </div>
          </Card>
        )}
        ...
        <Dialog open={!!onlinePayment} onOpenChange={(v) => !v && setOnlinePayment(null)}>
          <DialogContent className="max-w-sm">
            ...
            <p className="text-sm text-muted-foreground">
              Envía este link al cliente para que pague.
            </p>
            <div className="break-all rounded-lg border bg-muted/40 p-3 text-xs">
              {onlinePayment?.url}
            </div>
            <div className="flex gap-2">
              <Button
                ...
                onClick={async () => {
                  if (onlinePayment)
                    await navigator.clipboard.writeText(onlinePayment.url).catch(() => {});
              ...
              <Button
                className="flex-1"
                onClick={() => onlinePayment && window.open(onlinePayment.url, "_blank")}
              >
                Abrir link
              </Button>
            </div>
