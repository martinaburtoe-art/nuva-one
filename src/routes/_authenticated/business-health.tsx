...
function BusinessHealth() {
  ...
  const { data: customers = [], isLoading: customersLoading } = useBizList<any>("customers", {
    order: "name",
  });
  const { data: products = [], isLoading: productsLoading } = useBizList<any>("products", {
    order: "name",
  });
  const { data: activities = [], isLoading: activitiesLoading } = useBizList<any>(
    "customer_activities",
    { order: "created_at" },
  );
  const { data: cashFlow = [], isLoading: cashLoading } = useBizList<any>(
    "v_financial_cash_flow_daily",
    { order: "flow_date" },
  );
  const { data: reconciliation = [], isLoading: reconciliationLoading } = useBizList<any>(
  ...
  const overdueTasks = tasks.filter(
    (item: any) =>
      !item.completed &&
      item.due_date &&
      Number.isFinite(new Date(item.due_date).getTime()) &&
      new Date(item.due_date).getTime() < Date.now(),
  ).length;
  const intelligence = useMemo(
    () =>
      buildBusinessHealthIntelligence({
        sales,
        customers,
        products,
        activities,
          ...
          <div className="space-y-5">
            ...
            <Card className="p-5">
              ...
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                ...
                ejecución y controles. Es un indicador heurístico de gestión: no reemplaza estados
                financieros, asesoría profesional ni una predicción garantizada.
              </p>
            </Card>
          </div>
        )}
