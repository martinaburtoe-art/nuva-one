  ...
  return (
    <ModuleGuard module="analytics">
      <>
        ...
        /></div>
        <Card className="mt-4 p-4"><div className="flex flex-wrap items-end gap-3">
          ...
          {hasActiveFilters && <Button variant="ghost" size="sm" onClick={clearFilters}><X className="mr-1 h-3.5 w-3.5" /> Quitar filtros</Button>}
        </div></Card>
        <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-5">{kpis.map((k) => <Card key={k.l} className="p-5"><div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">{k.l}</span><k.i className="h-4 w-4 text-primary" /></div><div className="mt-2 text-2xl font-bold">{k.v}</div></Card>)}</div>
        <Card className="mt-6 p-6"><h3 className="font-semibold">Evolución de ventas</h3><ResponsiveContainer width="100%" height={260}><LineChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="oklch(0.92 0.008 270)" /><XAxis dataKey="fecha" stroke="oklch(0.5 0.02 270)" fontSize={12} /><YAxis stroke="oklch(0.5 0.02 270)" fontSize={12} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} /><Tooltip formatter={(v: number) => fmtCLP(v)} contentStyle={{ borderRadius: 12, border: "1px solid oklch(0.92 0.008 270)" }} /><Line type="monotone" dataKey="total" stroke="oklch(0.55 0.22 268)" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></Card>
        <div className="mt-6 grid gap-4 lg:grid-cols-2"><Card className="p-6"><h3 className="font-semibold">Ventas por canal</h3><ResponsiveContainer width="100%" height={280}><BarChart data={channelData}><CartesianGrid strokeDasharray="3 3" stroke="oklch(0.92 0.008 270)" /><XAxis dataKey="canal" stroke="oklch(0.5 0.02 270)" fontSize={12} /><YAxis stroke="oklch(0.5 0.02 270)" fontSize={12} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} /><Tooltip formatter={(v: number) => fmtCLP(v)} contentStyle={{ borderRadius: 12, border: "1px solid oklch(0.92 0.008 270)" }} /><Bar dataKey="total" fill="oklch(0.55 0.22 268)" radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></Card><Card className="p-6"><h3 className="font-semibold">Ventas por método de pago</h3><ResponsiveContainer width="100%" height={280}><BarChart data={paymentData}><CartesianGrid strokeDasharray="3 3" stroke="oklch(0.92 0.008 270)" /><XAxis dataKey="metodo" stroke="oklch(0.5 0.02 270)" fontSize={12} /><YAxis stroke="oklch(0.5 0.02 270)" fontSize={12} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} /><Tooltip formatter={(v: number) => fmtCLP(v)} contentStyle={{ borderRadius: 12, border: "1px solid oklch(0.92 0.008 270)" }} /><Bar dataKey="total" fill="oklch(0.6 0.18 200)" radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></Card></div>
      </>
    </ModuleGuard>
  );
}
