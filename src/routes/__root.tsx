    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          <button onClick={() => { router.invalidate(); reset(); }} className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">Reintentar</button>
          <a href="/" className="inline-flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 text-sm font-medium text-foreground hover:bg-accent">Inicio</a>
        </div>
      </div>
    </div>
  );
...
function PosScannerEnhancement() {
  const handleDetected = useCallback(async (code: string) => {
    ...
    const byBarcode = await supabase.from("products").select("id,name,sku,barcode").eq("business_id", businessId).eq("barcode", normalized).maybeSingle();
    if (byBarcode.data) product = byBarcode.data;
    if (!product) {
      const bySku = await supabase.from("products").select("id,name,sku,barcode").eq("business_id", businessId).eq("sku", normalized).maybeSingle();
      if (bySku.data) product = bySku.data;
    }
    if (!product) {
...
function RouteEnhancements() {
  ...
  useEffect(() => {
    ...
    mountNode.dataset.nuvaRouteEnhancement = isPos ? "pos-barcode-scanner" : "operating-pulse";
    const heading = Array.from(document.querySelectorAll("main h1")).find((node) => node.textContent?.trim() === (isPos ? "Caja" : "Clientes"));
    const pageHeader = heading?.parentElement?.parentElement;
    const content = pageHeader?.parentElement;
    if (!pageHeader || !content) return;
    mountNode.className = "mb-4 w-full";
    content.insertBefore(mountNode, pageHeader);
    setMount(mountNode);
    return () => { setMount(null); mountNode.remove(); };
  }, [location.pathname]);
