...
function setPosSearch(value: string) {
  const input = Array.from(document.querySelectorAll<HTMLInputElement>("input")).find((node) =>
    node.placeholder?.includes("Buscar por nombre, SKU o categoría"),
  );
...
function PosScannerEnhancement() {
  const handleDetected = useCallback(async (code: string) => {
    ...
    if (!businessId) {
      toast.error("Selecciona una empresa activa antes de escanear.");
      return;
    }
    const normalized = code.trim();
    let product: { id: string; name: string; sku: string | null; barcode: string | null } | null =
      null;
    ...
    if (!product) {
      ...
      toast.error(
        `No encontré un producto con código ${normalized}. Puedes asignarlo como código de barras en Inventario.`,
      );
      return;
    }
...
function RouteEnhancements() {
  ...
  useEffect(() => {
    ...
    const heading = Array.from(document.querySelectorAll("main h1")).find(
      (node) => node.textContent?.trim() === (isPos ? "Caja" : "Clientes"),
    );
  ...
  return createPortal(
    location.pathname === "/pos" ? <PosScannerEnhancement /> : <NuvaOperatingPulse />,
    mount,
  );
}
...
function LegacyFloatingAiCleanup() {
  useEffect(() => {
    const isLegacyAiButton = (element: Element) => {
      ...
      if (
        !/(nüva\s*ia|nuva\s*ia|nüva\s*intelligence|nuva\s*intelligence|abrir\s*ia|asistente\s*ia)/i.test(
          text,
        )
      )
        return false;
