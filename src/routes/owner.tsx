...
import { Activity, AlertTriangle, CheckCircle2, Clock3, Gauge, HeartPulse, LockKeyhole, RefreshCw, ServerCog, ShieldCheck, TriangleAlert, Wifi, XCircle } from "lucide-react";
...
function OwnerConsole() {
  ...
  return <div className="min-h-screen bg-[#07070c] px-4 py-6 text-white sm:px-8"><div className="mx-auto max-w-[1480px]">
    ...
    <section className="mt-5 grid gap-5 lg:grid-cols-2"><Panel title="Errores que requieren atención" icon={TriangleAlert} subtitle="Solo información técnica agregada; nunca contenido del usuario">{metrics?.top_errors.length ? <div className="space-y-2">{metrics.top_errors.map((item) => <div key={`${item.fingerprint}-${item.route}`} className="grid grid-cols-[1fr_auto] gap-4 rounded-xl border border-white/7 bg-black/10 p-3"><div><div className="font-mono text-xs text-amber-100/80">{item.fingerprint}</div><div className="mt-1 text-[11px] text-white/35">{item.route ?? "/"}{item.service ? ` · ${item.service}` : ""}</div></div><div className="text-sm font-semibold text-white/70">{item.count}×</div></div>)}</div> : <Empty title="Sin errores registrados" text="No hay eventos técnicos de error en la ventana de 24 horas."/>}</Panel><Panel title="Servicios" icon={ServerCog} subtitle="Actividad técnica agregada por servicio">{Object.keys(metrics?.services ?? {}).length ? <div className="space-y-2">{Object.entries(metrics?.services ?? {}).map(([name, value]) => <div key={name} className="flex items-center gap-3 rounded-xl border border-white/7 bg-black/10 p-3"><div className="min-w-0 flex-1"><div className="text-sm text-white/70">{name}</div><div className="mt-1 text-[11px] text-white/35">{value.requests ?? 0} eventos · {value.errors ?? 0} errores</div></div><div className="text-xs text-white/45">{value.avg_duration_ms != null ? `${Math.round(value.avg_duration_ms)} ms` : "—"}</div></div>)}</div> : <Empty title="Esperando telemetría" text="Los servicios aparecerán cuando exista actividad instrumentada."/>}</Panel></section>
    <section className="mt-5 grid gap-5 lg:grid-cols-2"><Panel title="Web Vitals" icon={Gauge} subtitle="Rendimiento observado cuando la aplicación reporta métricas"><div className="grid gap-3 sm:grid-cols-5"><Vital name="LCP" value={metrics?.vitals.lcp_ms} unit="ms"/><Vital name="INP" value={metrics?.vitals.inp_ms} unit="ms"/><Vital name="CLS" value={metrics?.vitals.cls} unit=""/><Vital name="FCP" value={metrics?.vitals.fcp_ms} unit="ms"/><Vital name="TTFB" value={metrics?.vitals.ttfb_ms} unit="ms"/></div></Panel><Panel title="Supervisión" icon={Clock3} subtitle="Señales para operación y respuesta a incidentes"><div className="grid gap-3 sm:grid-cols-2"><Mini title="Telemetría" value={metrics?.telemetry.source_available ? "Activa" : "Esperando eventos"}/><Mini title="Entorno" value={metrics?.environment ?? "production"}/><Mini title="Ventana" value="24 horas"/><Mini title="Próxima lectura" value="≤ 30 s"/></div></Panel></section>
...
function Vital({name,value,unit}:{name:string;value?:number|null;unit:string}){return <div className="rounded-xl border border-white/7 bg-black/10 p-3"><div className="text-[10px] font-bold tracking-wider text-white/35">{name}</div><div className="mt-2 text-sm font-semibold">{value == null ? "—" : `${value}${unit}`}</div></div>}
function Mini({title,value}:{title:string;value:string}){return <div className="rounded-xl border border-white/7 bg-black/10 p-3"><div className="text-[10px] uppercase tracking-wider text-white/30">{title}</div><div className="mt-2 text-sm font-semibold text-white/70">{value}</div></div>}
function Empty({title,text}:{title:string;text:string}){return <div className="rounded-2xl border border-dashed border-white/10 p-6 text-center"><div className="text-sm text-white/60">{title}</div><div className="mt-1 text-xs text-white/30">{text}</div></div>}
function StateBadge({state}:{state:"healthy"|"attention"|"critical"}){const label=state === "healthy" ? "Operativo" : state === "attention" ? "Atención" : "Incidente"; return <span className={`rounded-full border px-3 py-2 text-xs font-semibold ${state === "healthy" ? "border-emerald-300/15 bg-emerald-300/10 text-emerald-100" : state === "attention" ? "border-amber-300/15 bg-amber-300/10 text-amber-100" : "border-red-300/15 bg-red-300/10 text-red-100"}`}>{label}</span>}
