...
function OwnerConsole() {
  ...
  const cards = metrics ? [["Usuarios", metrics.users, Users], ["Empresas", metrics.businesses, Building2], ["Clientes", metrics.customers, Users], ["Productos", metrics.products, Gauge], ["Ventas", metrics.sales, Activity], ["Conversaciones IA", metrics.ai_conversations, Bot]] as const : [];
  return (
    <div className="min-h-screen bg-slate-950 px-4 py-6 text-white sm:px-8"><div className="mx-auto max-w-7xl">
      <header className="mb-8 flex flex-col gap-5 rounded-3xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between"><div><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.25em] text-cyan-200/70"><ShieldCheck className="h-4 w-4" /> Private Owner Console</div><h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">Nüva Pulse</h1><p className="mt-2 max-w-2xl text-sm text-white/55">El panel operativo de Nüva One: crecimiento, producto, IA, actividad y salud económica.</p></div><div className="flex gap-2"><button onClick={() => void refresh()} disabled={loading} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium transition hover:bg-white/10 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Actualizar</button><button onClick={() => void supabase.auth.signOut()} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium transition hover:bg-white/10"><LogOut className="h-4 w-4" /> Salir</button></div></header>
