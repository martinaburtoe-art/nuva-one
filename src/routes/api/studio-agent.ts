...
const TEXT_CAPABILITIES = new Set<AiCapability>([
  "chat", "research", "marketing", "copywriting", "brand", "strategy", "document",
]);
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!steps.every((step) => TEXT_CAPABILITIES.has(step.capability))) {
          return json({
            error: "El agente detectó una tarea multimedia. Esta versión ejecuta automáticamente capacidades de texto; imagen, voz y video siguen pasando por sus adapters especializados.",
            plan: { ...plan, steps },
            requires_specialized_media: true,
          }, 409);
        }
        ...
        for (let index = 0; index < steps.length; index += 1) {
          ...
          const { data: reservation, error: reserveError } = await supabase.rpc("reserve_ai_tool_quota" as never, { p_business_id: body.businessId, p_tool_id: toolId, p_units: tool.cost_units } as never);
          if (reserveError) return json({ error: "Se alcanzó el límite de uso de una de las herramientas del plan.", plan: { ...plan, steps }, completed: outputs }, 429);
          try {
            const dependencies = step.dependsOn.filter((n) => n >= 0 && n < outputs.length).map((n) => outputs[n]?.result).filter(Boolean).join("\n\n");
            const task = await runNuvaStudioTask({
              businessId: body.businessId,
              capability: step.capability,
              prompt: [`Objetivo: ${plan.goal}`, `Paso ${index + 1}: ${step.instruction}`, dependencies ? `Resultados previos:\n${dependencies}` : "", "Produce un entregable concreto y utilizable."].filter(Boolean).join("\n\n"),
              supabase,
            });
            outputs.push({ step: index, capability: step.capability, result: task.text });
          } catch (error) {
        ...
        return json({ ok: true, plan: { ...plan, steps }, outputs });
      },
    },
  },
});
