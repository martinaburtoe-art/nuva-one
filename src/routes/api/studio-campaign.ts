...
export const Route = createFileRoute("/api/studio-campaign")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const { data, error } = await a.supabase.from("nuva_studio_campaigns").select("*").eq("business_id", businessId).order("created_at", { ascending: false });
      ...
      PATCH: async ({ request }) => {
        ...
        const { data: campaign } = await a.supabase.from("nuva_studio_campaigns").select("id,user_id").eq("id", body.campaignId).maybeSingle();
        if (!campaign) return json({ error: "Campaña no encontrada" }, 404);
        if (campaign.user_id !== a.userId) return json({ error: "No tienes acceso a esta campaña" }, 403);
        const { data, error } = await a.supabase.from("nuva_studio_campaigns").update({ status: body.status, updated_at: new Date().toISOString(), ...(body.status === "active" ? { next_run_at: new Date().toISOString() } : {}) }).eq("id", body.campaignId).select("*").single();
        if (error) return json({ error: error.message }, 500);
