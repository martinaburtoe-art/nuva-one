...
export const Route = createFileRoute("/api/cron/nuva-intelligence-worker")({ server: { handlers: { GET: async ({ request }) => {
  ...
  const { url, anonKey, ok } = getServerSupabaseEnv(); const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY?.trim() ?? "";
  ...
  let scanned = 0, events = 0, risks = 0, opportunities = 0; const since = new Date(Date.now() - 24 * 3600 * 1000).toISOString();
  for (const business of businesses ?? []) {
    ...
    for (const signal of result.decision.signals) {
      ...
      if (!recent?.length) { const inserted = await db.from("nuva_intelligence_events").insert({ business_id: bid, event_type: signal.id, source_table: "nuva_operational_snapshot", severity: signal.severity, title: signal.title, summary: signal.explanation, evidence: { source: signal.source, metric: signal.metric, unit: signal.unit }, occurred_at: now }); if (!inserted.error) events++; }
      if (["critical", "warning"].includes(signal.severity)) {
        ...
        if (!existing?.length) { const inserted = await db.from("nuva_risks").insert({ business_id: bid, risk_type: signal.id, title: signal.title, description: signal.explanation, severity: signal.severity, probability: signal.severity === "critical" ? 0.8 : 0.55, impact: signal.metric ?? result.decision.score, confidence: 0.75, status: "open", evidence: { source: signal.source, metric: signal.metric, unit: signal.unit }, recommended_action: { action: signal.action }, detected_at: now }); if (!inserted.error) risks++; }
      }
      if (signal.severity === "opportunity") {
        ...
        if (!existing?.length) { const inserted = await db.from("nuva_opportunities").insert({ business_id: bid, opportunity_type: signal.id, title: signal.title, description: signal.explanation, potential_impact: signal.metric ?? result.decision.score, confidence: 0.7, status: "open", evidence: { source: signal.source, metric: signal.metric, unit: signal.unit }, recommended_action: { action: signal.action }, detected_at: now }); if (!inserted.error) opportunities++; }
      }
    }
    scanned++;
  }
