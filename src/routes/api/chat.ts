...
const SPECIALIST_PROFILES: Record<Specialist, string> = {
  orchestrator:
    "Eres el orquestador empresarial de Nüva One. Entiendes la empresa como un sistema integrado y decides qué área debe analizarse. Cruza ventas, compras, inventario, CRM y finanzas cuando los datos lo permitan. Entrega conclusiones ejecutivas, evidencia y próximos pasos.",
  finance:
    "Actúas como especialista en finanzas para PYMEs. Analiza liquidez, caja, ingresos, egresos, márgenes, rentabilidad, cuentas por cobrar/pagar, concentración y tendencias. Distingue siempre utilidad de flujo de caja y evita conclusiones que los datos no permitan.",
  accounting:
    "Actúas como especialista en contabilidad de gestión. Analiza cuentas, movimientos, débitos, créditos, saldos, asientos, estados financieros y consistencia contable usando exclusivamente los datos disponibles. No inventes asientos ni afirmes que algo fue contabilizado si no aparece en los datos.",
  ...
  sales:
    "Actúas como especialista en ventas. Analiza facturación, productos, clientes, frecuencia, ticket, conversión cuando exista y tendencias. Identifica oportunidades y problemas comerciales y propone acciones medibles.",
  inventory:
    "Actúas como especialista en inventario y abastecimiento. Analiza stock, rotación, productos, quiebres, sobrestock y compras cuando los datos existan. Propón reposición basada en evidencia y señala los datos que faltan para calcular un punto de pedido fiable.",
  ...
  strategy:
    "Actúas como consultor estratégico para PYMEs. Conecta desempeño financiero, comercial y operativo. Prioriza problemas por impacto y esfuerzo, formula hipótesis verificables y entrega un plan de acción concreto.",
};
...
function inferSpecialist(text: string): Specialist {
  ...
  if (
    /\b(iva|ppm|sii|tribut|f29|dte|factura electr|boleta|crédito fiscal|debito fiscal|débito fiscal)\b/.test(
      q,
    )
  )
    return "tax";
  if (
    /\b(contab|asiento|debe|haber|plan de cuentas|libro diario|libro mayor|balance general|balance de comprobación)\b/.test(
      q,
    )
  )
    return "accounting";
  if (
    /\b(caja|flujo|liquidez|rentab|margen|utilidad|ganancia|egreso|cuentas por cobrar|cuentas por pagar|presupuesto)\b/.test(
      q,
    )
  )
    return "finance";
  if (
    /\b(stock|inventario|inventarios|quiebre|sobrestock|rotación|rotacion|reponer|abastecimiento|proveedor)\b/.test(
      q,
    )
  )
    return "inventory";
  ...
  if (
    /\b(vend|ventas|factur|ticket|cotización|cotizacion|conversión|conversion|producto más vendido)\b/.test(
      q,
    )
  )
    return "sales";
  if (
    /\b(estrateg|plan de acción|plan de accion|crecer|decisión|decision|prioridad|objetivo|negocio)\b/.test(
      q,
    )
  )
    return "strategy";
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!withinChatRateLimit)
          return new Response(
            JSON.stringify({
              error: "El servicio está temporalmente ocupado. Intenta nuevamente en un minuto.",
            }),
        ...
        if (businessId) {
          ...
          if (usageError || allowed === false)
            return new Response(
              JSON.stringify({
                error: `Alcanzaste el límite de ${monthlyAiLimit} mensajes de IA de tu plan este mes. Actualiza tu plan para continuar.`,
              }),
              { status: 429 },
            );
        }
        ...
        const specialist =
          body.specialist && SPECIALIST_PROFILES[body.specialist]
            ? body.specialist
            : inferSpecialist(userText);
