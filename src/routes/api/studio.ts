  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const body = (await request.json().catch(() => null)) as { businessId?: string; capability?: AiCapability; prompt?: string } | null;
        ...
        try {
          if (body.capability === "image" || body.capability === "image_edit") {
            ...
            const { data: libraryAsset, error: assetError } = await supabase.from("ai_asset_library" as never).insert({
              business_id: body.businessId, user_id: userId, job_id: jobId, asset_type: "image",
              title: body.prompt.slice(0, 120), storage_path: asset.storagePath,
          ...
          if (body.capability === "voice") {
            ...
            const { data: libraryAsset, error: assetError } = await supabase.from("ai_asset_library" as never).insert({
              business_id: body.businessId, user_id: userId, job_id: jobId, asset_type: "audio",
              title: body.prompt.slice(0, 120), storage_path: asset.storagePath,
          ...
          await supabase.from("ai_generation_jobs" as never).update({ status: "completed", provider: result.metadata.provider, model: result.metadata.model, output_metadata: { ...result.metadata, quota: reservation }, completed_at: new Date().toISOString() } as never).eq("id", jobId);
          return json({ ok: true, result, usage: reservation });
        } catch (error) {
          await supabase.from("ai_generation_jobs" as never).update({ status: "failed", error_message: error instanceof Error ? error.message.slice(0, 1000) : "Error desconocido", completed_at: new Date().toISOString() } as never).eq("id", jobId);
          await supabase.rpc("release_ai_tool_quota" as never, { p_business_id: body.businessId, p_tool_id: toolId, p_units: unitsCharged } as never);
