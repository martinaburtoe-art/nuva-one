    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (usageError || allowed === false)
          return new Response(
            JSON.stringify({
              error: `Alcanzaste el límite de ${monthlyAiLimit} usos de IA de tu plan este mes. Actualiza tu plan para continuar.`,
            }),
            { status: 429 },
          );
...
${contextBlock}`;
        try {
          ...
          return new Response(JSON.stringify({ insights: parseInsights(result.text) }), {
            status: 200,
