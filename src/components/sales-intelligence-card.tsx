...
export function SalesIntelligenceCard({ sales, onAnalyze, onAskAI }: SalesIntelligenceCardProps) {
  ...
  const creditAmount = credit.reduce(
    (sum, s) => sum + Number(s.total || 0) - Number(s.paid_amount || 0),
    0,
  );
  ...
  const config = !hasData
    ? {
        label: "Observando",
        title: "Nüva todavía está aprendiendo de tus ventas",
        message: "Registra ventas para comenzar a detectar patrones comerciales.",
        icon: Sparkles,
        tone: "text-primary",
      }
    : attention
      ? {
          label: "Atención",
          title: "Tienes ventas a crédito que requieren seguimiento",
          message:
            "Nüva detectó saldo pendiente asociado a ventas a crédito. Prioriza el seguimiento para proteger tu flujo.",
          icon: AlertTriangle,
          tone: "text-warning",
        }
      : {
          label: "Oportunidad",
          title: "Tu actividad comercial está generando señales útiles",
          message:
            "Nüva recomienda observar el ticket promedio y los productos que más contribuyen al resultado.",
          icon: TrendingUp,
          tone: "text-success",
        };
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            {creditAmount > 0 && (
              <span className="rounded-full bg-warning/10 px-2.5 py-1 text-warning">
                Saldo a crédito: {fmtCLP(creditAmount)}
              </span>
            )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-muted-foreground">
                {attention
                  ? "Prioriza el seguimiento de ventas a crédito pendientes."
                  : "Revisa el rendimiento comercial y detecta qué está impulsando tus ventas."}
              </p>
            </div>
          </div>
