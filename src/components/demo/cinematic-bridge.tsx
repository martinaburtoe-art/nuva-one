          ...
          <Card className="relative overflow-hidden rounded-[1.5rem] border-border/70 bg-background/95 p-2 shadow-[0_30px_90px_-35px_hsl(var(--primary)/0.55)] sm:p-3">
            <div className="overflow-hidden rounded-[1.1rem] border bg-background">
              ...
              <div className="grid gap-4 p-4 sm:p-6 lg:grid-cols-[175px_1fr]">
                ...
                <div className="min-w-0"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><div className="text-xs text-muted-foreground">Alma Café · resumen del negocio</div><div className="mt-1 text-xl font-semibold">Buenos días 👋</div></div><div className="rounded-full border border-success/20 bg-success/5 px-3 py-1 text-xs text-success">Negocio saludable</div></div>
                  <div className="mt-5 grid grid-cols-2 gap-3 xl:grid-cols-4"><PreviewMetric icon={<TrendingUp />} label="Ventas" value="$359.910" trend="Período demo" /><PreviewMetric icon={<CircleDollarSign />} label="Caja" value="$189.910" trend="Resultado positivo" /><PreviewMetric icon={<Boxes />} label="Inventario" value="1 alerta" trend="Granos Colombia" /><PreviewMetric icon={<BarChart3 />} label="Nüva Score" value="78/100" trend="Saludable" /></div>
                  <div className="mt-4 grid gap-4 md:grid-cols-[1.45fr_1fr]"><div className="rounded-xl border p-4"><div className="flex items-center justify-between"><div><div className="text-sm font-medium">Ventas vs. gastos</div><div className="text-[11px] text-muted-foreground">Alma Café · período de demostración</div></div><BarChart3 className="h-4 w-4 text-muted-foreground" /></div><div className="mt-5 flex h-28 items-end gap-2">{[35,48,42,60,55,72,68,84,76,96].map((height, index) => <div key={index} className="flex h-full flex-1 items-end"><div className="w-full rounded-t bg-gradient-primary opacity-80 transition-transform duration-500 hover:scale-y-105" style={{ height: `${height}%` }} /></div>)}</div></div><div className="rounded-xl border bg-primary/[0.04] p-4"><div className="flex items-center gap-2 text-sm font-medium"><Bot className="h-4 w-4 text-primary" />Nüva IA</div><p className="mt-3 text-xs leading-5 text-muted-foreground">Detecté 1 producto cerca de su punto de reposición: Granos Colombia.</p><div className="mt-4 rounded-lg border bg-background/80 p-3 text-[11px]">Revisar inventario →</div></div></div>
                  <div className="mt-4 grid gap-3 sm:grid-cols-3"><MiniPill icon={<Boxes className="h-3.5 w-3.5" />} text="Ventas conectadas" /><MiniPill icon={<Users className="h-3.5 w-3.5" />} text="3 clientes" /><MiniPill icon={<Sparkles className="h-3.5 w-3.5" />} text="IA y recomendaciones" /></div>
                </div>
              </div>
            </div>
          </Card>
          ...
          <Card className="overflow-hidden border-primary/20 bg-gradient-to-br from-primary/[0.08] via-card to-card p-5 sm:p-6">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="w-full max-w-md rounded-2xl border bg-background/80 p-4 shadow-soft">
                ...
                <p className="mt-3 text-xs leading-5 text-muted-foreground">Granos Colombia tiene 6 unidades y su punto de reposición es 8. Nüva puede llevarte directamente al inventario para actuar.</p>
