...
// Solo se declaran masters realmente presentes en public/. Los capítulos restantes
// conservan el fallback editorial hasta que exista un master válido.
const REAL_MEDIA:Record<string,string>={scanner:"scan.mp4"};
...
export function SceneArt({kind,progress,reducedMotion=false,mediaMode="video"}:SceneArtProps){
 ...
 return <div className={`editorial-art editorial-art--${kind}`} style={style} aria-hidden="true">
  {showVideo&&<video ref={videoRef} className="editorial-art__video" src={`/home-cinematic/${mediaFile}`} muted playsInline preload="metadata" disablePictureInPicture onLoadedMetadata={()=>{const video=videoRef.current;if(video){try{video.currentTime=Math.max(0,Math.min(1,progress))*Math.max(0,video.duration-0.04)}catch{}}}} onError={()=>setVideoFailed(true)} aria-hidden="true"/>}
