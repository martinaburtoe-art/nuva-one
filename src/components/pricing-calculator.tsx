    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-5">
          ...
          <Card><CardHeader><CardTitle>IVA y presentación</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
            <Field label="IVA (%)"><Input type="number" min="0" max="100" value={Math.round(form.vatRate * 100)} onChange={(e) => set("vatRate", num(e.target.value) / 100)} /><Hint>Referencia general Chile: 19%.</Hint></Field>
            <Field label="Precio psicológico"><select value={form.psychologicalPricing ? "yes" : "no"} onChange={(e) => set("psychologicalPricing", e.target.value === "yes")} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="yes">Sí, usar terminaciones comerciales</option><option value="no">No, mantener precio exacto</option></select><Hint>En servicios B2B o premium suele ser preferible un precio redondo.</Hint></Field>
          </CardContent></Card>
        </div>
