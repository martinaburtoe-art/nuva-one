...
export function PricingCalculator() {
  ...
  const input = useMemo<PricingInput>(() => ({ ...form, competitorPrices: form.competitorPricesText.split(/[;,\s]+/).map(Number).filter((v) => v > 0), elasticity: form.elasticityText.trim() ? Number(form.elasticityText) : null }), [form]);
  ...
  async function loadProduct() {
    if (!productCode.trim()) { toast.error("Ingresa un SKU, código o código de barras."); return; }
