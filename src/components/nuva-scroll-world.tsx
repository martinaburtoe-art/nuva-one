...
function LaptopScene({ phase }: { phase: number }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-[#070809]">
      ...
      <div
        className="absolute left-1/2 top-[11%] w-[78%] -translate-x-1/2 sm:top-[12%] sm:w-[60%]"
          ...
          <div className="relative h-full overflow-hidden rounded-[.72rem] border border-black bg-[#050607] shadow-[inset_0_0_45px_rgba(0,0,0,.95)]">
            <div
              ...
              style={{
                opacity: screenGlow,
                background:
                  "radial-gradient(circle at 70% 0%, hsl(var(--primary) / .35), transparent 48%)",
              }}
            />
            <div className="absolute inset-[7%] flex flex-col">
              ...
              <div className="mt-[10%] max-w-[68%]">
                <p className="text-[clamp(5px,.65vw,9px)] uppercase tracking-[.32em] text-primary/80">
                  Todo conectado
                </p>
                <p className="mt-[4%] text-[clamp(1rem,2.8vw,3rem)] font-medium leading-[.91] tracking-[-.055em] text-white">
                  Tu negocio genera datos.
                </p>
                <p className="text-[clamp(1rem,2.8vw,3rem)] font-medium leading-[.91] tracking-[-.055em] text-white/40">
      ...
      <div
        className="absolute bottom-[2%] h-[31%] w-[22%] rounded-[48%_44%_35%_42%] border border-white/[.06] bg-[radial-gradient(circle_at_34%_22%,rgba(236,220,208,.22),transparent_30%),linear-gradient(145deg,rgba(193,171,157,.2),rgba(78,65,59,.08))] shadow-[15px_20px_55px_rgba(0,0,0,.55)] blur-[.15px]"
        style={{ left: `${fingerX}%`, transform: "rotate(-13deg)" }}
      >
      ...
      <div className="absolute bottom-[7%] left-[6%] text-[9px] uppercase tracking-[.34em] text-white/25">
        scroll · entrar
      </div>
      <div className="absolute bottom-[7%] right-[6%] text-[9px] uppercase tracking-[.25em] text-white/20">
        01 / 08
      </div>
    </div>
  );
  ...
  const title =
    {
      ...
      score: "Nüva Score",
      decision: "Decisión",
    } satisfies Partial<Record<SceneId, string>>;
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-[#07090c] p-5 sm:p-10">
      ...
      <div className="relative z-10 flex items-center justify-between border-b border-white/10 pb-4">
        ...
        <span className="text-[9px] uppercase tracking-[.25em] text-white/25">{title[id] ?? "Nüva One"}</span>
      </div>
      ...
      {id === "sales" && (
        <div className="relative z-10 h-[78%] pt-8">
          <div className="mb-7 flex items-end justify-between">
            <div>
              <p className="text-xs text-white/35">Ventas hoy</p>
              <p className="mt-1 text-4xl tracking-[-.04em] sm:text-6xl">$428.600</p>
            </div>
            <span className="rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-[10px] text-primary">+18,4%</span>
          </div>
          <div className="flex h-[68%] items-end gap-1.5 sm:gap-2">
            {[31, 46, 39, 59, 52, 73, 66, 87, 78, 94, 82, 100].map((height, i) => (
              <div key={i} className="relative flex-1 rounded-t bg-primary/45" style={{ height: `${Math.min(100, height + phase * 4)}%`, opacity: 0.42 + i * 0.038 }}>
                {i === 11 && <span className="absolute -top-6 right-0 text-[9px] text-primary">ahora</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      {id === "inventory" && (
        <div className="relative z-10 flex h-[78%] items-center justify-center">
          <div className="relative flex aspect-video w-full max-w-3xl items-center justify-center overflow-hidden rounded-[1.4rem] border border-primary/25 bg-black/45 shadow-[inset_0_0_80px_rgba(0,0,0,.7)]">
            ...
            <div className="absolute bottom-5 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full border border-white/10 bg-black/80 px-4 py-2 text-xs text-primary backdrop-blur-md">
      ...
      {id === "finance" && (
        <div className="relative z-10 pt-10">
          <div className="grid gap-4 sm:grid-cols-3">
            {[["Disponible", "$3.420.000"], ["Margen", "24,8%"], ["Proyección", "+12,6%"]].map(([label, value], i) => (
              <div key={label} className={`rounded-2xl border p-5 ${i === 2 ? "border-primary/15 bg-primary/[.05]" : "border-white/10 bg-white/[.025]"}`}>
                <p className="text-xs text-white/30">{label}</p>
                <p className={`mt-2 text-3xl tracking-[-.04em] sm:text-4xl ${i === 2 ? "text-primary" : ""}`}>{value}</p>
              </div>
            ))}
          </div>
          <div className="mt-10 h-48 rounded-2xl border border-white/10 bg-black/20 p-4">
            <svg viewBox="0 0 900 180" className="h-full w-full">
              <defs><linearGradient id="nuva-finance-fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="currentColor" stopOpacity=".18" /><stop offset="100%" stopColor="currentColor" stopOpacity="0" /></linearGradient></defs>
              <path d="M0 148 C90 142 110 110 205 122 S310 140 390 78 S510 108 610 60 S760 78 900 18 L900 180 L0 180 Z" fill="url(#nuva-finance-fill)" className="text-primary" />
              <path d="M0 148 C90 142 110 110 205 122 S310 140 390 78 S510 108 610 60 S760 78 900 18" fill="none" stroke="currentColor" strokeWidth="3" className="text-primary/60" />
            </svg>
          </div>
        </div>
      )}
      ...
      {id === "crm" && (
        <div className="relative z-10 grid gap-3 pt-10 sm:grid-cols-3">
          {[["Camila R.", "8 compras", "Alta recurrencia"], ["Felipe M.", "5 compras", "Buen margen"], ["Antonia S.", "4 compras", "Lista para volver"]].map(([name, purchases, note]) => (
            <div key={name} className="rounded-2xl border border-white/10 bg-white/[.025] p-6" style={{ transform: `translateY(${(1 - phase) * 10}px)` }}>
              <span className="grid size-11 place-items-center rounded-full bg-primary/10 text-sm text-primary">{name[0]}</span>
              <p className="mt-7 text-lg">{name}</p>
              <p className="mt-1 text-xs text-white/35">{purchases} · cliente recurrente</p>
              <p className="mt-6 border-t border-white/10 pt-4 text-[10px] uppercase tracking-[.18em] text-white/25">{note}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {id === "ai" && (
        <div className="relative z-10 mx-auto max-w-4xl pt-8">
          <div className="rounded-2xl border border-white/10 bg-white/[.025] p-5 text-base text-white/65 shadow-[0_20px_70px_rgba(0,0,0,.2)]">¿Qué debería hacer esta semana para mejorar mis ventas?</div>
          <div className="mt-3 rounded-2xl border border-primary/15 bg-primary/[.07] p-6 text-sm leading-7 text-white/75"><span className="mb-3 block text-[9px] uppercase tracking-[.25em] text-primary">Recomendación de Nüva</span>Prioriza reponer <strong className="text-white">SKU-1048</strong>, contacta a tus clientes recurrentes y concentra la promoción en los productos con mayor margen. Nüva cruza ventas, inventario, clientes y finanzas para llegar a esta recomendación.</div>
          <div className="mt-4 flex flex-wrap gap-2 text-[10px] text-white/30">{["Ventas", "Inventario", "Clientes", "Finanzas"].map((item) => <span key={item} className="rounded-full border border-white/10 px-3 py-1">{item}</span>)}</div>
        </div>
      )}

      {id === "score" && (
        <div className="relative z-10 flex h-[78%] flex-col items-center justify-center text-center">
          <p className="text-[10px] uppercase tracking-[.3em] text-white/30">Salud del negocio</p>
          <p className="mt-5 text-[clamp(7rem,20vw,12rem)] font-medium leading-none tracking-[-.08em] text-white">82</p>
          <p className="mt-3 text-sm text-white/40">de 100 · negocio saludable</p>
          <div className="mt-8 h-1.5 w-[min(70vw,460px)] overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-primary/70 shadow-[0_0_22px_hsl(var(--primary)/.45)]" style={{ width: `${82 * clamp(phase * 1.35)}%` }} /></div>
        </div>
      )}
    </div>
  );
...
function Decision({ progress }: { progress: number }) {
  return (
    <div className="absolute inset-0 flex items-center justify-center overflow-hidden bg-[#020304] px-6 text-center">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_40%,hsl(var(--primary)/.1),transparent_38%)]" />
      <div className="relative max-w-6xl">
        <p className="text-[10px] uppercase tracking-[.4em] text-white/30">NÜVA ONE · SIGUIENTE MOVIMIENTO</p>
        <h2 className="mt-7 text-[clamp(3.1rem,8vw,8.5rem)] font-medium leading-[.88] tracking-[-.07em] text-white">Tu negocio genera datos.<br /><span className="text-white/40">Nüva los convierte en decisiones.</span></h2>
        <p className="mx-auto mt-8 max-w-2xl text-sm leading-6 text-white/40">De la venta al inventario. Del inventario a las finanzas. De los datos a una decisión concreta.</p>
        <Link to="/auth" className="group mt-9 inline-flex items-center gap-3 rounded-full bg-white px-6 py-3 text-sm font-semibold text-black shadow-[0_15px_50px_rgba(255,255,255,.12)]">Empieza con Nüva <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></Link>
        <div className="mt-10 text-[9px] tracking-[.3em] text-white/20">{Math.round(progress * 100)}% · EXPERIENCIA COMPLETA</div>
      </div>
    </div>
  );
}
