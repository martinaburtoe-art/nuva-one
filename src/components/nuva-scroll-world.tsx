...
function AmbientScene({ phase, dark = false }: { phase: number; dark?: boolean }) {
  ...
  return (
    <div className={`absolute inset-0 overflow-hidden rounded-[2rem] ${dark ? "bg-[#030407]" : "bg-[#111318]"}`}>
      ...
      <div className="absolute -top-[18%] h-[70%] w-[45%] rounded-full bg-primary/[.07] blur-[90px] transition-transform duration-300" style={{ left: `${lightX}%` }} />
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-[1.8rem] border border-white/15 bg-[#07090d] shadow-[0_50px_130px_rgba(0,0,0,.78)] [transform:perspective(1200px)_rotateX(2deg)]">
      ...
      <div className="relative flex h-full flex-col p-4 sm:p-8">
        <div className="flex items-center justify-between border-b border-white/10 pb-3"><div className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary/10"><scene.icon className="h-4 w-4 text-primary" /></span><div><p className="text-xs font-semibold text-white">Nüva One</p><p className="text-[8px] text-white/30">Inteligencia conectada</p></div></div><span className="text-[8px] text-primary">nuva.one</span></div>
...
function Layer({ scene, distance, focusDepth }: { scene: Scene; distance: number; focusDepth: number }) {
  ...
  const transform = screen
    ? `translateZ(${scene.z * 600}px) translate3d(${entering ? 0 : distance < -120 ? -14 : 14}px,0,0) scale(${scale})`
    : `translateZ(${scene.z * 600}px) scale(${scale})`;
  return <div className="absolute inset-[5%]" style={{ transformStyle: "preserve-3d", transform, opacity, filter: blur > .4 ? `blur(${blur}px)` : undefined, pointerEvents: focus > .65 ? "auto" : "none" }}>
    ...
    {entering && screen && <div className="pointer-events-none absolute inset-0 rounded-[2rem] border border-primary/35 shadow-[0_0_80px_hsl(var(--primary)/.16)]" />}
  </div>;
}
...
export function NuvaScrollWorld() {
  ...
  return <section ref={sectionRef} className="relative bg-[#05070a] text-white" aria-label="Experiencia cinematográfica de Nüva One">
    <div style={{ height: "840vh" }}>
      <div className="sticky top-0 flex h-screen min-h-[40rem] items-center overflow-hidden py-8 sm:py-12">
        ...
        <div className="relative mx-auto w-full max-w-[1440px] px-3 sm:px-6">
          ...
          <div className="relative mx-auto h-[29rem] w-full max-w-6xl overflow-hidden rounded-[2rem] border border-white/10 bg-[#030507] shadow-[0_40px_140px_rgba(0,0,0,.72)] sm:h-[36rem]" style={{ perspective: mobile ? "850px" : "1500px" }}>
            <div className="absolute inset-0" style={{ transformStyle: "preserve-3d", transform: `rotateX(${cameraRotateX}deg) rotateY(${cameraRotateY}deg) translateZ(${-cameraZ + 430}px)` }}>
