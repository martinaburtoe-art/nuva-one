    ...
    <div className="relative h-full w-full overflow-hidden bg-[#07090c] p-5 sm:p-10">
      ...
      {id === "sales" && (
        <div className="relative z-10 h-[78%] pt-8">
          <div className="mb-7 flex items-end justify-between">
            ...
            <span className="rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-[10px] text-primary">
              +18,4%
            </span>
          </div>
          <div className="flex h-[68%] items-end gap-1.5 sm:gap-2">
            {[31, 46, 39, 59, 52, 73, 66, 87, 78, 94, 82, 100].map((height, i) => (
              <div
                key={i}
                className="relative flex-1 rounded-t bg-primary/45"
                style={{
                  height: `${Math.min(100, height + phase * 4)}%`,
                  opacity: 0.42 + i * 0.038,
                }}
              >
                {i === 11 && (
                  <span className="absolute -top-6 right-0 text-[9px] text-primary">ahora</span>
                )}
              </div>
            ))}
          </div>
      ...
      {id === "inventory" && (
        <div className="relative z-10 flex h-[78%] items-center justify-center">
          <div className="relative flex aspect-video w-full max-w-3xl items-center justify-center overflow-hidden rounded-[1.4rem] border border-primary/25 bg-black/45 shadow-[inset_0_0_80px_rgba(0,0,0,.7)]">
            ...
            <div
              className="absolute inset-x-[9%] h-px bg-primary shadow-[0_0_34px_hsl(var(--primary)/.95)]"
              style={{ top: `${8 + phase * 84}%` }}
            />
      ...
      {id === "finance" && (
        <div className="relative z-10 pt-10">
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ["Disponible", "$3.420.000"],
              ["Margen", "24,8%"],
              ["Proyección", "+12,6%"],
            ].map(([label, value], i) => (
              <div
                key={label}
                className={`rounded-2xl border p-5 ${i === 2 ? "border-primary/15 bg-primary/[.05]" : "border-white/10 bg-white/[.025]"}`}
              >
                <p className="text-xs text-white/30">{label}</p>
                <p
                  className={`mt-2 text-3xl tracking-[-.04em] sm:text-4xl ${i === 2 ? "text-primary" : ""}`}
                >
                  {value}
                </p>
              </div>
          ...
          <div className="mt-10 h-48 rounded-2xl border border-white/10 bg-black/20 p-4">
            <svg viewBox="0 0 900 180" className="h-full w-full">
              <defs>
                <linearGradient id="nuva-finance-fill" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="currentColor" stopOpacity=".18" />
                  <stop offset="100%" stopColor="currentColor" stopOpacity="0" />
                </linearGradient>
              </defs>
              <path
                d="M0 148 C90 142 110 110 205 122 S310 140 390 78 S510 108 610 60 S760 78 900 18 L900 180 L0 180 Z"
                fill="url(#nuva-finance-fill)"
      ...
      {id === "crm" && (
        <div className="relative z-10 grid gap-3 pt-10 sm:grid-cols-3">
          {[
            ["Camila R.", "8 compras", "Alta recurrencia"],
            ["Felipe M.", "5 compras", "Buen margen"],
            ["Antonia S.", "4 compras", "Lista para volver"],
          ].map(([name, purchases, note]) => (
            <div
              key={name}
              className="rounded-2xl border border-white/10 bg-white/[.025] p-6"
              style={{ transform: `translateY(${(1 - phase) * 10}px)` }}
            >
              <span className="grid size-11 place-items-center rounded-full bg-primary/10 text-sm text-primary">
                {name[0]}
              </span>
              <p className="mt-7 text-lg">{name}</p>
              <p className="mt-1 text-xs text-white/35">{purchases} · cliente recurrente</p>
              <p className="mt-6 border-t border-white/10 pt-4 text-[10px] uppercase tracking-[.18em] text-white/25">
                {note}
              </p>
            </div>
          ))}
      ...
      {id === "ai" && (
        <div className="relative z-10 mx-auto max-w-4xl pt-8">
          <div className="rounded-2xl border border-white/10 bg-white/[.025] p-5 text-base text-white/65 shadow-[0_20px_70px_rgba(0,0,0,.2)]">
            ¿Qué debería hacer esta semana para mejorar mis ventas?
          </div>
          <div className="mt-3 rounded-2xl border border-primary/15 bg-primary/[.07] p-6 text-sm leading-7 text-white/75">
            <span className="mb-3 block text-[9px] uppercase tracking-[.25em] text-primary">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-[10px] text-white/30">
            {["Ventas", "Inventario", "Clientes", "Finanzas"].map((item) => (
      ...
      {id === "score" && (
        <div className="relative z-10 flex h-[78%] flex-col items-center justify-center text-center">
          ...
          <p className="mt-5 text-[clamp(7rem,20vw,12rem)] font-medium leading-none tracking-[-.08em] text-white">
            82
          </p>
          ...
          <div className="mt-8 h-1.5 w-[min(70vw,460px)] overflow-hidden rounded-full bg-white/10">
            <div
              className="h-full rounded-full bg-primary/70 shadow-[0_0_22px_hsl(var(--primary)/.45)]"
              style={{ width: `${82 * clamp(phase * 1.35)}%` }}
            />
          </div>
        </div>
      )}
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center overflow-hidden bg-[#020304] px-6 text-center">
      ...
      <div className="relative max-w-6xl">
        ...
        <Link
          to="/auth"
          className="group mt-9 inline-flex items-center gap-3 rounded-full bg-white px-6 py-3 text-sm font-semibold text-black shadow-[0_15px_50px_rgba(255,255,255,.12)]"
        >
          Empieza con Nüva{" "}
      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <div className="absolute inset-0 [perspective:1400px]">
          ...
          >
            ...
            >
              <LaptopScene phase={clamp(progress * 1.8)} />
            </div>
            {scenes.slice(1, 7).map((scene) => {
              ...
              return (
                <div
                  key={scene.id}
                  className="absolute left-1/2 top-0 h-screen w-screen -translate-x-1/2 will-change-transform"
                  style={{
                    transform: `translateZ(${scene.depth}px) translateY(${lift}px)`,
                    opacity,
                    filter: `blur(${Math.min(7, distance / 135)}px)`,
                    zIndex: scene.depth <= z ? 2 : 1,
                  }}
                >
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div
                      className="relative h-[78vh] w-[min(94vw,1180px)] overflow-hidden rounded-[1.6rem] border border-white/15 shadow-[0_70px_180px_rgba(0,0,0,.86)] will-change-transform"
                      style={{ transform: `scale(${scale})`, transformOrigin: "center center" }}
                    >
                      <AppScreen id={scene.id} phase={near} />
                      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(115deg,transparent_35%,rgba(255,255,255,.06)_47%,transparent_58%)] opacity-30" />
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="pointer-events-none absolute left-5 top-1/2 hidden -translate-y-1/2 md:block">
          <div className="space-y-3">
            {scenes.map((scene) => (
              <div
                key={scene.id}
                className={`h-1 rounded-full transition-all duration-300 ${active.id === scene.id ? "w-10 bg-white" : "w-3 bg-white/20"}`}
              />
            ))}
          </div>
        </div>
        <div className="pointer-events-none absolute left-1/2 top-6 -translate-x-1/2 rounded-full border border-white/10 bg-black/30 px-4 py-2 text-[9px] uppercase tracking-[.3em] text-white/35 backdrop-blur-md">
          {active.label}
        </div>
