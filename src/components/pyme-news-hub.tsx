  ...
  const load = async () => {
    ...
    await Promise.all(FEEDS.map(async (feed) => {
      try {
        ...
        const rss = data as RssPayload;
        const rawItems = Array.isArray(rss.items) ? rss.items : [];
        rawItems.slice(0, 5).forEach((raw) => {
          if (!isRssItem(raw)) return;
    ...
    <Card className="overflow-hidden border-primary/15 bg-gradient-to-br from-primary/[0.04] via-background to-background">
      <div className="border-b p-5 md:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="rounded-xl bg-primary/10 p-2 text-primary"><BellRing className="h-5 w-5" /></div>
            <div>
              <div className="flex items-center gap-2"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Nüva PYME Radar</p><Badge variant="secondary" className="gap-1 text-[10px]"><Sparkles className="h-3 w-3" /> Actualización automática</Badge></div>
              <h2 className="mt-1 text-lg font-semibold">Noticias y oportunidades que pueden mover tu negocio</h2>
              <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Fondos, cambios tributarios, leyes, programas públicos, digitalización y oportunidades relevantes para PYMEs chilenas.</p>
            </div>
          </div>
      ...
      <div className="grid gap-3 p-5 md:grid-cols-2 md:p-6 lg:grid-cols-4">
        {visible.map((item, idx) => {
          const Icon = item.source === "SII" ? Scale : item.source === "Corfo" ? Landmark : WalletCards;
          return <article key={`${item.link}-${idx}`} className="group rounded-xl border bg-background/75 p-4 transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-soft">
            <div className="flex items-center justify-between gap-2"><Badge variant="outline" className="gap-1"><Icon className="h-3 w-3" />{item.source}</Badge><span className="text-[10px] text-muted-foreground">{item.pubDate ? new Date(item.pubDate).toLocaleDateString("es-CL") : "Actual"}</span></div>
            <h3 className="mt-3 line-clamp-3 text-sm font-semibold leading-5">{item.title}</h3>
            {item.summary && <p className="mt-2 line-clamp-3 text-xs leading-5 text-muted-foreground">{item.summary}</p>}
            <a href={item.link} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center text-xs font-semibold text-primary">Ver fuente oficial <ExternalLink className="ml-1 h-3 w-3" /></a>
          </article>;
        })}
      </div>
