...
import { AlertTriangle, ArrowUpRight, CheckCircle2, Lightbulb, Loader2, ShieldAlert } from "lucide-react";
...
const actionHref = (id: string) => {
  if (id === "low-stock") return "/inventory";
  if (id === "receivables-overdue") return "/crm";
  if (id === "purchase-pressure") return "/purchases";
  if (id === "cash-burn") return "/finance";
  if (id === "growth-opportunity") return "/customers";
  return "/dashboard";
};
  ...
  const { data: actions = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const signals = buildNuvaIntelligence({
        ...
        stock: (products.data ?? []).map((p: any) => ({ ...p, quantity: p.stock })),
      });
      return buildNuvaActionCenter(signals, 5);
    },
  });
  ...
  return (
    <Card className="mt-6 overflow-hidden border-primary/20 bg-gradient-to-br from-primary/[0.06] via-background to-accent/30 p-6 shadow-soft">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Link to="/executive-command-center"><Button variant="outline" size="sm">Ver centro ejecutivo <ArrowUpRight className="ml-1 h-4 w-4" /></Button></Link>
      </div>
      ...
      {isLoading ? (
        <div className="mt-5 flex items-center gap-2 rounded-xl border bg-background/70 p-4 text-sm text-muted-foreground" role="status">
          <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> Analizando tu negocio…
        </div>
      ) : (
            ...
            return (
              <div key={item.id} className="rounded-xl border bg-background/80 p-4 transition-all hover:-translate-y-0.5 hover:shadow-sm">
                <div className="flex items-start gap-3">
                  <div className="mt-0.5 rounded-lg bg-accent p-2"><Icon className="h-4 w-4" aria-hidden="true" /></div>
                  <div className="min-w-0 flex-1">
                    ...
                    <Link to={actionHref(item.id)} className="mt-3 inline-flex items-center text-sm font-medium text-primary hover:underline">{item.action} <ArrowUpRight className="ml-1 h-3.5 w-3.5" /></Link>
                  </div>
                </div>
              </div>
            );
