      ...
      <div className="px-4 py-4 md:px-5 md:py-5">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
              ...
              <span className="hidden text-xs text-muted-foreground sm:inline">
                Señales de tu cartera y pipeline en tiempo real
              </span>
            </div>
          </div>
        ...
        <div className="mt-4 grid grid-cols-2 divide-x divide-y overflow-hidden rounded-lg border bg-muted/20 md:grid-cols-4 md:divide-y-0">
          ...
          <Metric
            ...
            value={crm.pendingTasks}
            critical={crm.pendingTasks > 0}
          />
        </div>
      </div>
      ...
      <div className="border-t bg-muted/[0.08] px-4 py-4 md:px-5">
        ...
        <div className="grid gap-2 md:grid-cols-3">
          {signals.map((signal) => (
            <div
              key={signal.title}
              className="flex min-h-[104px] flex-col rounded-lg border bg-background p-3"
            >
              <div className="flex items-start justify-between gap-2">
                <div
                  className={`rounded-md p-1.5 ${signal.tone === "critical" ? "bg-destructive/10 text-destructive" : signal.tone === "attention" ? "bg-amber-500/10 text-amber-600" : "bg-success/10 text-success"}`}
                >
              ...
              <Link
                to={signal.href}
                className="mt-auto pt-2 text-[11px] font-semibold text-primary"
              >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t px-4 py-2.5 md:px-5">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <PhoneCall className="h-3 w-3" /> {crm.active} activos
          </span>
          <span className="flex items-center gap-1.5">
            <Mail className="h-3 w-3" /> {crm.openQuotes} cotizaciones abiertas
          </span>
          <span>
            Ventas asociadas:{" "}
        ...
        <Button asChild size="sm" variant="ghost" className="h-7 px-2 text-xs">
          <Link to="/customers">
            Gestionar CRM <ArrowUpRight className="ml-1 h-3 w-3" />
          </Link>
        </Button>
      </div>
    </Card>
