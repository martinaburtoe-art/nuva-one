...
export function NuvaOperatingPulse() {
  ...
  const crm = useMemo(() => {
    ...
    const customerSales = (sales ?? []).filter((s: any) => !!s.customer_id);
    const revenue = customerSales.reduce((sum: number, s: any) => sum + (Number(s.total) || 0), 0);
    return {
      total: list.length,
      active,
      prospects,
  ...
  const signals = useMemo<Signal[]>(() => {
    ...
    if (crm.pendingTasks) {
      result.push({
        ...
        detail: `${crm.pendingTasks} tarea(s) de clientes requieren atención inmediata.`,
        href: "/customers",
        tone: "critical",
      });
    }
    if (crm.stale) {
      result.push({
        title: "Clientes en riesgo de enfriarse",
        detail: `${crm.stale} cliente(s) llevan más de 14 días sin contacto registrado.`,
        href: "/customers",
        tone: "attention",
      });
    }
    if (crm.openQuotes) {
      result.push({
        title: "Cotizaciones por convertir",
        detail: `${crm.openQuotes} cotización(es) permanecen abiertas y pueden requerir seguimiento comercial.`,
        href: "/quotes",
        tone: "attention",
      });
    }
    if (!result.length) {
      result.push({
        title: "Relación comercial bajo control",
        detail: "No detectamos seguimientos vencidos ni señales críticas en tu cartera.",
        href: "/customers",
        tone: "positive",
      });
    }
    ...
    <Card className="mb-6 overflow-hidden border-primary/15 bg-gradient-to-br from-background via-primary/[0.025] to-background">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-primary">
              <WalletCards className="h-4 w-4" /> Nüva Operating Pulse
            </div>
            <h2 className="mt-1 text-lg font-semibold">Inteligencia comercial de tus clientes</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Detecta oportunidades, riesgos y seguimientos antes de que se conviertan en ventas perdidas.
            </p>
          </div>
        ...
        {signals.map((signal) => (
          <div key={signal.title} className="rounded-xl border bg-background/75 p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <Badge
                variant={signal.tone === "critical" ? "destructive" : signal.tone === "attention" ? "secondary" : "outline"}
              >
            ...
            <Link to={signal.href} className="mt-3 inline-flex items-center text-xs font-semibold text-primary">
              Resolver ahora <ArrowUpRight className="ml-1 h-3.5 w-3.5" />
            </Link>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t bg-muted/20 px-5 py-3 md:px-6">
        <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5"><PhoneCall className="h-3.5 w-3.5" /> {crm.active} clientes activos</span>
          <span className="flex items-center gap-1.5"><Mail className="h-3.5 w-3.5" /> {crm.openQuotes} cotizaciones abiertas</span>
        ...
        <Button asChild size="sm" variant="outline">
          <Link to="/customers">Gestionar CRM <ArrowUpRight className="ml-1 h-3.5 w-3.5" /></Link>
        </Button>
      </div>
    </Card>
  );
  ...
  return (
    <div className="rounded-xl border bg-background/70 p-3">
      <div className="flex items-center gap-2">
        <Icon className={`h-4 w-4 ${critical ? "text-destructive" : "text-primary"}`} />
