...
export function NuvaLaunchExperience({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div aria-label="Bienvenido a Nüva One" aria-live="polite"
      ...
      style={{ "--nuva-exit-ms": `${EXIT_MS}ms` } as CSSProperties}>
      <style>{`
        /* One source of truth: neutralize the legacy flight animation. */
        .nuva-launch .nuva-launch__module { animation: none !important; }
        .nuva-launch--intro .nuva-launch__module { opacity: 0; filter: blur(10px); transform: translate(-50%,-50%) rotate(var(--nuva-angle)) translateY(0) rotate(calc(-1 * var(--nuva-angle))) scale(.22); }
        .nuva-launch--orbit .nuva-launch__module { opacity: 1; filter: blur(0); transform: translate(-50%,-50%) rotate(var(--nuva-angle)) translateY(calc(-1 * var(--nuva-radius))) rotate(calc(-1 * var(--nuva-angle))) scale(1); transition: transform 1450ms cubic-bezier(.16,1,.3,1), opacity 950ms cubic-bezier(.16,1,.3,1), filter 950ms cubic-bezier(.16,1,.3,1); transition-delay: var(--nuva-delay); }
        .nuva-launch--converge .nuva-launch__module { opacity: .12; filter: blur(5px); transform: translate(-50%,-50%) rotate(var(--nuva-angle)) translateY(0) rotate(calc(-1 * var(--nuva-angle))) scale(.35); transition: transform ${CONVERGE_MS}ms cubic-bezier(.76,0,.24,1), opacity ${CONVERGE_MS}ms ease, filter ${CONVERGE_MS}ms ease; transition-delay: calc(var(--nuva-delay) * .35); }
        .nuva-launch--welcome .nuva-launch__module,.nuva-launch--exit .nuva-launch__module { opacity: 0; filter: blur(9px); transform: translate(-50%,-50%) rotate(var(--nuva-angle)) translateY(0) rotate(calc(-1 * var(--nuva-angle))) scale(.2); transition: opacity 850ms ease, filter 850ms ease, transform 1000ms cubic-bezier(.22,1,.36,1); }
        .nuva-launch--intro .nuva-launch__core { transform: scale(.9); opacity: .78; }
        .nuva-launch--orbit .nuva-launch__core { transform: scale(1); opacity: 1; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 700ms ease; }
        .nuva-launch--converge .nuva-launch__core { transform: scale(1.06); transition: transform ${CONVERGE_MS}ms cubic-bezier(.16,1,.3,1); }
        .nuva-launch--welcome .nuva-launch__core { transform: scale(.92); opacity: .35; transition: transform 800ms ease, opacity 700ms ease; }
        ...
        .nuva-launch--welcome .nuva-launch__welcome{opacity:1;visibility:visible;animation:nuva-welcome-in 900ms cubic-bezier(.16,1,.3,1) both}
        ...
        .nuva-launch--exit .nuva-launch__grid,.nuva-launch--exit .nuva-launch__light-beam,.nuva-launch--exit .nuva-launch__ring,.nuva-launch--exit .nuva-launch__orbit,.nuva-launch--exit .nuva-launch__core,.nuva-launch--exit .nuva-launch__tagline{opacity:.12;filter:blur(3px);transition:opacity ${EXIT_MS}ms cubic-bezier(.22,1,.36,1),filter ${EXIT_MS}ms cubic-bezier(.22,1,.36,1),transform ${EXIT_MS}ms cubic-bezier(.22,1,.36,1);transform:scale(1.04)}
        ...
        @keyframes nuva-welcome-out{0%{opacity:1;transform:translate(-50%,-50%) scale(1);filter:blur(0)}45%{opacity:.72;transform:translate(-50%,-50%) scale(1.012);filter:blur(1px)}100%{opacity:0;transform:translate(-50%,-50%) scale(1.045);filter:blur(10px)}}
        @keyframes nuva-scene-out{0%{opacity:1;transform:scale(1);filter:blur(0)}45%{opacity:.9;transform:scale(1.008);filter:blur(.5px)}100%{opacity:0;transform:scale(1.035);filter:blur(7px)}}
      ...
      <div className="nuva-launch__stage">
        <div className="nuva-launch__orbit" aria-hidden="true">{items.map(([label, Icon, angle], index) => <div key={label} className="nuva-launch__module" style={{"--nuva-angle":`${angle}deg`,"--nuva-delay":`${index*95}ms`} as CSSProperties}><div className="nuva-launch__module-icon"><Icon /></div><span>{label}</span></div>)}</div>
