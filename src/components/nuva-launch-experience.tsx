...
export function NuvaLaunchExperience({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      try {
        const sec = Math.min(DURATION, Math.max(0, now - start)), p = sec / DURATION;
        ...
        modules.forEach((el, index) => {
          ...
          set(el, opacity, `translate(-50%,-50%) translate(${x}px,${y}px) scale(${Math.max(.05, scale)})`, `blur(${blur}px)`);
        });
        ...
        if (sec >= DURATION) { if (!finished) { finished = true; completeRef.current(); } return; }
        frame = requestAnimationFrame(tick);
      } catch (error) {
        console.error("Nüva launch animation recovered from runtime error", error);
        modules.forEach((el) => set(el, 1, "translate(-50%,-50%) scale(1)", "none"));
        set(core, 1, "translate(-50%,-50%) scale(1)");
        set(logo, 1, "scale(1)");
        frame = requestAnimationFrame(tick);
      }
    };
    // Never leave the launch empty while hydration or the first animation frame is pending.
    modules.forEach((el) => set(el, .72, "translate(-50%,-50%) scale(.72)", "blur(2px)"));
    set(core, .18, "translate(-50%,-50%) scale(.78)");
    set(halo, .16, "scale(.68)", "blur(12px)");
    set(ring, .16, "scale(.78)");
    set(pulse, .12, "scale(.94)");
    set(logo, .16, "scale(.82)", "blur(4px)");
    set(welcome, .01, "translateX(-50%) translateY(12px) scale(.9)");
    set(tagline, .01, "translateX(-50%) translateY(8px)");
    frame = requestAnimationFrame(tick);
