...
const EXIT = 900;
const ITEMS = [
  ["IA", Bot], ["Clientes", Users], ["Ventas", ShoppingCart], ["Finanzas", CircleDollarSign], ["Caja", ReceiptText],
  ["Compras", ClipboardList], ["Inventario", Package], ["CRM", Handshake], ["Gestión", BriefcaseBusiness], ["Datos", Boxes], ["Insights", ChartNoAxesCombined],
] as const;
...
export function NuvaLaunchExperience({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!root) return;
    const q = (selector: string): HTMLElement | null => root.querySelector(selector) as HTMLElement | null;
    const modules = Array.from(root.querySelectorAll<HTMLElement>("[data-nuva-v2-module]"));
    const core = q("[data-nuva-v2-core]");
    const halo = q("[data-nuva-v2-halo]");
    const ring = q("[data-nuva-v2-ring]");
    const pulse = q("[data-nuva-v2-pulse]");
    const logo = q("[data-nuva-v2-logo]");
    const welcome = q("[data-nuva-v2-welcome]");
    const tagline = q("[data-nuva-v2-tagline]");
    const outer = q("[data-nuva-v2-outer]");
    const inner = q("[data-nuva-v2-inner]");
    const ambient = q("[data-nuva-v2-ambient]");
    const beam1 = q("[data-nuva-v2-beam='1']");
    const beam2 = q("[data-nuva-v2-beam='2']");
    const start = performance.now();
    let frame = 0;
    let finished = false;

    const set = (el: HTMLElement | null, opacity: number, transform = "none", filter = "none") => {
      if (!el) return;
      el.style.setProperty("opacity", String(clamp(opacity)), "important");
    ...
    const tick = (now: number) => {
      ...
      if (ambient) {
        ambient.style.setProperty("opacity", String(0.5 + 0.38 * Math.sin(p * Math.PI)), "important");
        ambient.style.setProperty("transform", `scale(${0.96 + 0.12 * p})`, "important");
      }
      set(outer, elapsed < 1300 ? ease(elapsed / 1300) : 0.72, `translate(-50%,-50%) scale(${0.72 + 0.42 * smooth(p)})`);
      set(inner, elapsed < 1600 ? ease(elapsed / 1600) : 0.76, `translate(-50%,-50%) scale(${0.72 + 0.42 * smooth(p)}) rotate(${360 * p}deg)`);
      set(beam1, elapsed > 2600 && elapsed < 11800 ? 0.34 : 0, `translate(-50%,-50%) rotate(${18 + 42 * p}deg) scaleX(${0.7 + p})`);
      set(beam2, elapsed > 3400 && elapsed < 11800 ? 0.24 : 0, `translate(-50%,-50%) rotate(${-18 - 40 * p}deg) scaleX(${0.7 + p})`);

      const coreIn = ease(elapsed / 1900);
      const coreOut = smooth((elapsed - 11600) / 2200);
      const coreVisible = elapsed < 11600 ? coreIn : 1 - coreOut;
      set(core, coreVisible, `translate(-50%,-50%) scale(${0.7 + 0.32 * coreIn})`);
      set(halo, coreVisible * 0.95, `scale(${0.58 + 0.72 * smooth(elapsed / 11200)})`, "blur(12px)");
      set(ring, coreVisible, `scale(${0.72 + 0.25 * smooth(p)}) rotate(${360 * p}deg)`);
      set(pulse, coreVisible * (0.25 + 0.4 * Math.abs(Math.sin(elapsed / 650))), `scale(${1 + 0.08 * Math.sin(elapsed / 520)})`);

      const logoIn = ease((elapsed - 900) / 1700);
      const logoOut = smooth((elapsed - 11600) / 2200);
      set(logo, logoIn * (1 - logoOut), `scale(${0.74 + 0.28 * logoIn})`, elapsed < 1500 ? "blur(7px)" : "none");
      ...
      modules.forEach((el, index) => {
        ...
        const radius = Math.min(window.innerWidth * 0.32, 245) * (1 - 0.86 * converge);
        const x = Math.cos(angle) * radius;
        const y = Math.sin(angle) * radius;
        const opacity = inP < 1 ? Math.max(0.02, inP) : Math.max(0, 1 - smooth((elapsed - 11300) / 1700));
        const scale = inP < 1 ? 0.18 + 0.82 * inP : Math.max(0.08, 1 - 0.9 * converge);
        const blur = inP < 1 ? (1 - inP) * 8 : Math.max(0, converge - 0.78) * 25;
        set(el, opacity, `translate(-50%,-50%) translate(${x}px,${y}px) scale(${scale})`, `blur(${blur}px)`);
      });

      const wi = smooth((elapsed - 7400) / 1200);
      const wo = smooth((elapsed - 11900) / 1700);
      set(welcome, wi * (1 - wo), `translateX(-50%) translateY(${(1 - wi) * 14 - wo * 10}px) scale(${0.9 + 0.1 * wi})`);
      const ti = smooth((elapsed - 3900) / 1000);
      const to = smooth((elapsed - 11800) / 1600);
      set(tagline, ti * (1 - to), `translateX(-50%) translateY(${(1 - ti) * 8 - to * 8}px)`);

      if (elapsed >= DURATION) {
        if (!finished) { finished = true; completeRef.current(); }
        return;
      }
  ...
  return (
    <div ref={rootRef} className={`nuva-launch-v2${exiting ? " nuva-launch-v2--exit" : ""}`} aria-label="Bienvenido a Nüva One" aria-live="polite">
      ...
      <div className="nuva-launch-v2__stage">
        <div className="nuva-launch-v2__orbit" aria-hidden="true">
          {ITEMS.map(([label, Icon]) => <div key={label} data-nuva-v2-module className="nuva-launch-v2__module"><div className="nuva-launch-v2__icon"><Icon /></div><span>{label}</span></div>)}
        </div>
        <div data-nuva-v2-core className="nuva-launch-v2__core">
          ...
          <div data-nuva-v2-logo className="nuva-launch-v2__logo"><span>Nüva</span><small>ONE</small></div>
        </div>
        <div data-nuva-v2-tagline className="nuva-launch-v2__tagline"><Sparkles /> Todo tu negocio. Una inteligencia.</div>
        <div data-nuva-v2-welcome className="nuva-launch-v2__welcome"><div className="nuva-launch-v2__welcome-title">Bienvenido a Nüva One</div><div className="nuva-launch-v2__welcome-subtitle">Todo tu negocio. Una inteligencia.</div></div>
      </div>
