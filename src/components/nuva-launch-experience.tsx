...
@media(prefers-reduced-motion:reduce){.nuva-launch__module,.nuva-launch__welcome,.nuva-launch__core,.nuva-launch__ring,.nuva-launch__tagline{animation:none!important;opacity:1!important;filter:none!important}}
`;
...
export function NuvaLaunchExperience({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(onComplete, media.matches ? 700 : DURATION);
