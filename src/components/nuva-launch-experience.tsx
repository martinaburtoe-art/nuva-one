...
export function NuvaLaunchExperience({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    let frame = 0;
    const set = (el: HTMLElement | null, opacity: number, transform = "none", filter = "none") => { if (!el) return; el.style.opacity = String(Math.max(0, Math.min(1, opacity))); el.style.transform = transform; el.style.filter = filter; };
    const tick = (now: number) => {
      const sec = Math.min(DURATION, now - start), p = sec / DURATION;
      if (ambient) { ambient.style.opacity = String(0.55 + 0.35 * Math.sin(Math.min(1, p) * Math.PI)); ambient.style.transform = `scale(${.96 + .12 * p})`; }
      ...
      modules.forEach((el, index) => {
        ...
        el.style.transform = `translate(-50%,-50%) translate(${x}px,${y}px) scale(${Math.max(.05, scale)})`;
