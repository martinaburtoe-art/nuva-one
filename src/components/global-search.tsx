...
export function GlobalSearch({ visibleNav }: { visibleNav?: readonly { to: string }[] }) {
  ...
  const mobileTrigger = mounted
    ? createPortal(
        <button
          type="button"
          aria-label={online ? "Abrir búsqueda y acciones rápidas" : "Abrir acciones rápidas sin conexión"}
        ...
        >
          ...
          {!online && <span className="absolute -right-0.5 -top-0.5 flex h-3 w-3 rounded-full bg-destructive ring-2 ring-background" aria-hidden="true" />}
  ...
  return (
    <>
      ...
      <button
        type="button"
        aria-label={online ? "Abrir búsqueda y acciones rápidas" : "Abrir búsqueda y acciones rápidas; sin conexión"}
        onClick={() => setOpen(true)}
      ...
      >
        ...
        {!online && <WifiOff className="ml-2 hidden h-4 w-4 shrink-0 text-destructive md:block" aria-label="Sin conexión" />}
        <CommandShortcut className="ml-2 hidden md:inline-flex">⌘K</CommandShortcut>
      </button>

      <CommandDialog open={open} onOpenChange={setOpen}>
        <CommandInput placeholder={online ? "Buscar módulos, SKU, scanner o acciones..." : "Sin conexión: acciones locales y navegación disponible..."} />
        <CommandList>
          ...
          {!online && (
            <CommandGroup heading="Estado de conexión">
              <CommandItem value="sin conexión" disabled>
                ...
                Sin conexión a Internet. Las operaciones que requieren servidor podrán reintentarse al recuperar la conexión.
              </CommandItem>
            </CommandGroup>
          )}
          {actions.length > 0 && (
