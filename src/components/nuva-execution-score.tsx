...
export function NuvaExecutionScore({ activities = [], priorities = 0 }: Props) {
  ...
  const discipline = tasks.length
    ? Math.max(0, Math.round(((completed - overdue) / tasks.length) * 100))
    : 0;
  const score = tasks.length
    ? Math.max(0, Math.min(100, Math.round(execution * 0.7 + discipline * 0.3)))
    : 0;
  const label =
    score >= 80
      ? "Excelente ejecución"
      ...
      <div className="p-6 md:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-primary">
              <Target className="h-4 w-4" /> Nüva Execution Score
            </div>
            <h3 className="mt-2 text-xl font-semibold tracking-tight">
              Convierte recomendaciones en resultados
            </h3>
            <p className="mt-1 text-sm text-muted-foreground">
              Mide qué tan bien se están ejecutando las acciones comerciales.
            </p>
          </div>
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl border bg-background/80 text-2xl font-bold tabular-nums">
            {score}
          </div>
        </div>
        ...
        <div className="mt-5 rounded-xl border bg-background/70 p-4">
          ...
          <p className="mt-3 text-xs leading-5 text-muted-foreground">
            El score combina cumplimiento de tareas y penaliza seguimientos vencidos. A medida que
            ejecutes acciones, Nüva podrá medir mejor tu disciplina comercial.
          </p>
        </div>
      </div>
    </Card>
