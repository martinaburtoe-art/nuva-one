...
export function PurchasesProductAnalytics({ purchases, products }: Props) {
  ...
  const suppliers = useMemo(
    () =>
      Array.from(
        new Set((purchases ?? []).map((p: any) => p.supplier_name).filter(Boolean)),
      ).sort(),
    [purchases],
  );
  ...
  const rows = useMemo(() => {
    ...
    const grouped = new Map<
      string,
      { name: string; sku: string; category: string; units: number; spend: number; orders: number }
    >();
    ...
    for (const purchase of purchases ?? []) {
      ...
      for (const item of items) {
        ...
        const productCategory = String(
          item.category || product?.category || purchase.category || "Sin categoría",
        );
  ...
  const metricLabel =
    metric === "spend" ? "Gasto" : metric === "orders" ? "Órdenes" : "Unidades compradas";
      ...
      <div className="border-b p-5 md:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-primary">
              <BarChart3 className="h-4 w-4" /> Análisis de compras
            </div>
            <h2 className="mt-1 text-xl font-semibold tracking-tight">Productos más comprados</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Identifica qué productos estás reponiendo, cuánto gastas y qué proveedores concentran
              tus compras.
            </p>
          </div>
          {hasActiveFilters && (
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <Select value={metric} onValueChange={(v) => setMetric(v as Metric)}>
              ...
              <SelectContent>
                <SelectItem value="units">Unidades compradas</SelectItem>
                <SelectItem value="spend">Gasto generado</SelectItem>
                <SelectItem value="orders">Número de órdenes</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
        ...
        <div className="mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <div className="mt-1 flex h-10 items-center rounded-md border px-3 text-xs text-muted-foreground">
                ...
                {rows.map((row, index) => (
                  <tr key={`${row.name}-${index}`} className="border-t">
                    ...
                    <td className="px-4 py-3 text-right tabular-nums">
                      {row.units.toLocaleString("es-CL")}
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">{fmtCLP(row.spend)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
