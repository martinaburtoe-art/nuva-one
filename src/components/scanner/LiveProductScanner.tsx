...
export function LiveProductScanner({
  open,
  onOpenChange,
  onScan,
  onResolved,
  ...
  const emitAction = useCallback(
    (action: ScannerAction, product?: NonNullable<ProductResolution["product"]>) => {
  ...
  const handleDetection = useCallback(
    async (result: UnifiedScanResult) => {
      ...
      try {
        ...
        window.setTimeout(() => {
          ...
          if (continuous && scannerRef.current && state !== "stopped") setState("scanning");
        }, 900);
      }
    },
    [onCodeNotFound, onProductFound, onResolved, continuous, state],
  );
  ...
  const submitManualCode = async () => {
    ...
    if (!code) return;
    await handleDetection({ rawValue: code, format: "manual", input: "manual" } as UnifiedScanResult);
  };

  const statusText = {
    idle: "Listo para escanear",
    requesting_permission: "Solicitando acceso a la cámara…",
    camera_starting: "Iniciando cámara…",
    scanning: continuous ? "Apunta al código · escaneo continuo" : "Apunta al código",
    detected: "Código detectado",
    resolving: "Buscando producto…",
    found: "Producto encontrado",
    not_found: "Código nuevo",
    duplicate: "Múltiples coincidencias",
    inactive: "Código inactivo",
    permission_denied: "Permiso de cámara denegado",
    camera_error: "No se pudo iniciar la cámara",
    unsupported: "Este dispositivo no permite escaneo en vivo",
      ...
      <DialogContent className="max-w-lg overflow-hidden p-0">
        <DialogHeader className="px-4 pt-4">
          <DialogTitle className="flex items-center gap-2">
            ...
            {title}
            {continuous && <Badge variant="outline" className="ml-auto text-[10px]">Continuo</Badge>}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-3 px-4 pb-4">
          <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-black">
            ...
            <div className="absolute left-3 top-3">
              <Badge variant="secondary" className="gap-1 bg-black/55 text-white">
                <span className="h-2 w-2 animate-pulse rounded-full bg-primary" />
                {statusText}
              </Badge>
            </div>
            {state === "resolving" && (
              <div className="absolute inset-x-0 bottom-3 mx-auto flex w-fit items-center gap-2 rounded-full bg-black/65 px-3 py-2 text-xs text-white">
          ...
          <div className="flex gap-2">
            <Input
              ...
              placeholder="Ingresar SKU / EAN manualmente"
              className="font-mono"
              inputMode="text"
            />
            <Button size="icon" variant="outline" onClick={() => void submitManualCode()} disabled={!manualCode.trim()} aria-label="Buscar código">
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0 flex-1">
              <div className="text-xs text-muted-foreground">Último código</div>
              <div className="truncate font-mono text-sm font-semibold">{lastCode || "—"}</div>
              {lastFormat && <div className="text-[11px] text-muted-foreground">{lastFormat}</div>}
            </div>
            <div className="flex gap-2">
              <Button size="icon" variant="outline" disabled={!torchSupported} onClick={toggleTorch} aria-label="Linterna">
                {torch ? <ZapOff className="h-4 w-4" /> : <Zap className="h-4 w-4" />}
              </Button>
              <Button size="icon" variant="outline" onClick={() => { stop(); onOpenChange(false); }} aria-label="Cerrar">
                <X className="h-4 w-4" />
              </Button>
            </div>
          </div>
          ...
          {state === "found" && (
            <div className="space-y-2 rounded-xl border border-primary/30 bg-primary/5 p-3">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                <Button size="sm" variant="outline" onClick={() => emitAction("entry")}><PackagePlus className="mr-1.5 h-4 w-4" />Entrada</Button>
                <Button size="sm" variant="outline" onClick={() => emitAction("exit")}><PackagePlus className="mr-1.5 h-4 w-4" />Salida</Button>
                <Button size="sm" variant="outline" onClick={() => emitAction("count")}><ClipboardList className="mr-1.5 h-4 w-4" />Contar</Button>
                <Button size="sm" variant="outline" onClick={() => emitAction("add_code")}><Plus className="mr-1.5 h-4 w-4" />Agregar código</Button>
                <Button size="sm" variant="outline" onClick={() => emitAction("view_product")}><Search className="mr-1.5 h-4 w-4" />Ver producto</Button>
              </div>
            </div>
          )}
          ...
          {state === "not_found" && (
            <div className="rounded-xl border bg-muted/40 p-3 text-sm">
              ...
              <div className="mt-1 text-xs text-muted-foreground">Puedes convertir este código en un producto nuevo o asociarlo posteriormente a un producto existente.</div>
              <div className="mt-3 flex gap-2">
                <Button size="sm" onClick={() => emitAction("new_product")}><Plus className="mr-1.5 h-4 w-4" />Crear producto</Button>
          ...
          {(state === "duplicate" || state === "camera_error" || state === "permission_denied" || state === "unsupported") && (
            <div className="rounded-xl border bg-muted/40 p-3 text-sm">
              ...
              {state === "duplicate" && <div className="mt-1 text-xs text-muted-foreground">Revisa los productos asociados y conserva un único código primario por producto.</div>}
          ...
          {state === "scanning" && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <Camera className="h-4 w-4" />Cámara en vivo · también acepta lectores HID · puedes ingresar el código manualmente
            </div>
          )}
          {state === "stopped" && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <CameraOff className="h-4 w-4" />Cámara detenida
            </div>
          )}
        </div>
      </DialogContent>
