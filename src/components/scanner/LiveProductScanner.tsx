...
import {
  executeScannerInventoryAction,
  type ScannerInventoryAction,
} from "@/lib/scanner-inventory-actions";
...
import { createProductFromScanner as createProductFromScannerRpc } from "@/lib/scanner-product-onboarding";
...
export type ScannerAction =
  ...
  | "add_code"
  | "view_product";
type ScannerProduct = NonNullable<ProductResolution["product"]> & {
  id: string;
  stock?: number | null;
};
type NewProductForm = {
...
export type LiveProductScannerProps = {
  ...
  onAction?: (
    ...
    context: { code: string; product?: ScannerProduct; result?: UnifiedScanResult },
  ) => void;
  title?: string;
  continuous?: boolean;
};
...
export function LiveProductScanner({
      ...
      if ((action === "entry" || action === "exit" || action === "count") && product?.id) {
        ...
        setReason(
          action === "entry"
            ...
            : action === "exit"
              ? "Salida desde escáner"
              : "Conteo físico desde escáner",
        );
      }
    ...
    async (result: UnifiedScanResult) => {
      ...
      try {
        ...
        window.setTimeout(() => {
          ...
          if (continuousRef.current && scannerRef.current && !operation && !newProductOpen)
            setState("scanning");
        }, 900);
      }
    },
  ...
  const submitOperation = async () => {
    ...
    try {
      const validated = validateScannerInventoryAction({
        operation,
        quantity: parsedQuantity,
        reason,
        currentStock: Number(operationProduct.stock ?? 0),
      });
      ...
      const stockBefore =
        typeof payload?.stock_before === "number" ? payload.stock_before : validated.currentStock;
      ...
      toast.success(
        `Movimiento registrado · stock ${stockBefore} → ${stockAfter ?? "actualizado"}`,
      );
      setOperation(null);
      setQuantity("1");
      setReason("");
  ...
  const openNewProduct = () => {
    if (!canWrite || !businessId || !lastCode) {
      toast.error("No se puede crear el producto sin una empresa y un código válido.");
      return;
    }
  ...
  const createProductFromScanner = async () => {
    ...
    if (
      ![cost, price, initialStock].every(Number.isFinite) ||
      cost < 0 ||
      price < 0 ||
      !Number.isInteger(initialStock) ||
      initialStock < 0
    ) {
      toast.error("Costo, precio y stock inicial deben ser valores válidos.");
      return;
    }
    ...
    try {
      ...
      if (existing.status === "FOUND")
        throw new Error(
          `El código ya está asociado a ${existing.product?.name ?? "otro producto"}.`,
        );
      if (existing.status === "DUPLICATE")
        throw new Error("El código tiene múltiples asociaciones y no puede reutilizarse.");
