...
export function LandingMotion() {
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              entry.target.classList.add("nuva-revealed");
      ...
      const parallax = () => {
        ...
        raf = requestAnimationFrame(() => {
          parallaxTargets.forEach((el) => {
            ...
            el.style.setProperty("--nuva-parallax", `${Math.max(-18, Math.min(18, offset))}px`);
          });
        });
      };
  ...
  useEffect(() => {
    ...
    const updateHover = (event: MouseEvent) => {
      ...
      setCursorHover(Boolean(target?.closest("a, button, [role=button], input, textarea, select")));
    };
  ...
  return (
    <>
      <style>{`
        ...
        .nuva-magnetic { transition: transform .2s cubic-bezier(.22,1,.36,1), box-shadow .2s ease; }
        .nuva-magnetic:hover { transform: translateY(-2px); box-shadow: 0 14px 30px hsl(var(--primary) / .18); }
      ...
      {loading && (
        <div className="fixed inset-0 z-[10001] flex items-center justify-center bg-background" aria-label="Cargando Nüva One">
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="h-1 w-24 overflow-hidden rounded-full bg-secondary"><div className="h-full w-1/2 animate-[nuvaLoader_0.9s_ease-in-out_infinite] rounded-full bg-primary" /></div>
          </div>
        </div>
      )}
