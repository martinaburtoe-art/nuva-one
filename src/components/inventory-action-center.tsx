...
function priority(product: Product) {
  ...
  const suffix = quantity > 0 ? `${quantity} unidad${quantity === 1 ? "" : "es"}` : "configurar objetivo";
  if (m.health === "out_of_stock") return { label: "Crítica", className: "bg-destructive/10 text-destructive", action: `Reponer ${suffix}` };
  if (m.health === "critical") return { label: "Alta", className: "bg-warning/15 text-warning", action: `Programar reposición de ${suffix}` };
  if (m.health === "reorder") return { label: "Media", className: "bg-primary/10 text-primary", action: `Preparar reposición de ${suffix}` };
...
export function InventoryActionCenter({ products, canWrite = true }: Props) {
  ...
  const candidates = products
    ...
    .sort((a, b) => {
      ...
      return aMetrics.available - bMetrics.available || bMetrics.suggestedOrder - aMetrics.suggestedOrder;
    });
  ...
  async function createAction(product: Product) {
    ...
    try {
      await insertActivity.mutateAsync({
        ...
        due_date: new Date(Date.now() + (m.available === 0 ? 86400000 : 2 * 86400000)).toISOString(),
        completed: false,
      });
      toast.success(m.suggestedOrder > 0 ? `Acción creada: reponer ${m.suggestedOrder} unidad${m.suggestedOrder === 1 ? "" : "es"}` : "Acción creada: configurar objetivo de reposición");
    } catch {
        ...
        <div className="p-6 md:p-7">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">La prioridad considera stock disponible real, reservado, bloqueado y unidades en tránsito. La reposición sugerida no es un pronóstico: se basa en parámetros configurados.</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button size="sm" variant="outline" onClick={() => navigate({to:"/inventario-conteo"})}><ScanBarcode className="mr-2 h-4 w-4"/>Escanear / registrar</Button>
            </div>
          </div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-4">
            ...
            <Metric icon={<Package className="h-4 w-4"/>} label="Sin disponible" value={String(criticalCount)}/>
          </div>

          <div className="mt-5 space-y-3">
            {candidates.length === 0 ? (
              <div className="rounded-xl border border-dashed p-5 text-sm text-muted-foreground">No hay productos en zona de reposición.</div>
                ...
                <div key={product.id} className="rounded-xl border bg-background/75 p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">Físico {m.stock} · disponible {m.available} · reservado {m.reserved} · bloqueado {m.blocked} · tránsito {m.inTransit} · proyectado {m.projected}</p>
                      <p className="mt-2 text-xs font-medium text-primary">Recomendación: {p.action} · objetivo {m.target || "no definido"} · sugerido {m.suggestedOrder}</p>
                    </div>
          ...
          <p className="mt-4 text-xs leading-5 text-muted-foreground">Este panel no modifica stock. Los movimientos reales deben pasar por el flujo transaccional de inventario para conservar stock anterior/nuevo y auditoría.</p>
        </div>
      </Card>
