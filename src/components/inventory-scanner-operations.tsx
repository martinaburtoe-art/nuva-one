...
export function InventoryScannerOperations() {
  ...
  async function resolve(codeValue: string) {
    ...
    try {
      ...
      if (result.status !== "FOUND" || !result.product)
        return toast.error("Producto no encontrado. Créalo desde el conteo físico o el catálogo.");
  ...
  async function apply() {
    ...
    if (!Number.isSafeInteger(qty) || qty <= 0)
      return toast.error("La cantidad debe ser un entero mayor que cero.");
    ...
    try {
      ...
      toast.success(
        operation === "entry" || operation === "receipt"
          ? `Entrada de ${qty} unidad(es) registrada.`
          : `Salida de ${qty} unidad(es) registrada.`,
      );
      setOpen(false);
      setProduct(null);
      setCode("");
  ...
  const operationLabel =
    operation === "entry" ? "Entrada" : operation === "receipt" ? "Recepción" : "Salida";
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Escanea un producto para registrar entradas o salidas de stock con trazabilidad.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" disabled={!canWrite} onClick={() => setScannerOpen(true)}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{operationLabel} de inventario</DialogTitle>
          </DialogHeader>
          {product && (
            <div className="space-y-4">
              <div className="rounded-lg border p-4">
                ...
                <div className="mt-1 text-sm text-muted-foreground">
                  SKU: {product.sku ?? "—"} · Código: {product.barcode ?? code} · Stock actual:{" "}
                  {Number(product.stock ?? 0)}
                </div>
              </div>
              <div>
                <Label htmlFor="inventory-operation-quantity">Cantidad</Label>
                <Input
                  id="inventory-operation-quantity"
                  type="number"
                  min="1"
                  step="1"
                  value={quantity}
              ...
              <div>
                <Label htmlFor="inventory-operation-reason">Motivo</Label>
                <Input
                  id="inventory-operation-reason"
                  value={reason}
                  onChange={(e) => setReason(e.target.value)}
                  placeholder={
                    operation === "entry" ? "Ej. recepción de proveedor" : "Ej. salida por merma"
                  }
                />
              </div>
              <Badge variant="outline" className="flex w-fit items-center gap-1">
                <Settings2 className="h-3 w-3" />
                Movimiento auditado y atómico
              </Badge>
              <Button className="w-full" disabled={busy} onClick={() => void apply()}>
                {busy ? "Registrando…" : `Confirmar ${operationLabel.toLowerCase()}`}
              </Button>
            </div>
          )}
        </DialogContent>
