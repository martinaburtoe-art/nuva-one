  ...
  const filtered = useMemo(() => {
    ...
    return codes.filter((c) => {
      ...
      return (
        ...
        c.code_type.toLowerCase().includes(q) ||
  ...
  async function save() {
    ...
    try {
      ...
      if (resolution.status === "DUPLICATE")
        return toast.error(
          "Este código tiene múltiples asociaciones y debe resolverse antes de guardarlo.",
        );
      if (resolution.status === "FOUND" && resolution.product?.product_id !== productId)
        return toast.error(
          `Este código ya pertenece a ${resolution.product?.name ?? "otro producto"}.`,
        );
      ...
      if (nextSku && nextSku !== product?.sku?.toUpperCase()) {
        const { error: skuError } = await (supabase as any)
          .from("products")
          .update({ sku: nextSku })
          .eq("id", productId)
          .eq("business_id", businessId);
      ...
      toast.error(error?.message ?? "No se pudo guardar el código.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <Card className="overflow-hidden border-primary/15">
      <div className="p-5 md:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-primary">
              <Barcode className="h-4 w-4" /> Códigos y SKU
            </div>
            <h2 className="mt-2 text-xl font-semibold">Identificación profesional de productos</h2>
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              SKU interno, EAN/UPC/QR y códigos alternativos con un único lector en vivo.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={() => setScanOpen(true)} disabled={!canWrite}>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2">
          ...
          <Badge variant="outline">{primaryCount} principales</Badge>
        </div>
        <div className="mt-4 overflow-x-auto rounded-xl border">
          {loadingCodes ? (
            <div className="p-6 text-sm text-muted-foreground">Cargando códigos…</div>
          ) : !filtered.length ? (
            <div className="p-8 text-center text-sm text-muted-foreground">
              <Tag className="mx-auto mb-2 h-7 w-7" />
              Aún no hay códigos externos registrados.
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {filtered.slice(0, 50).map((c) => {
                  ...
                  return (
                    <TableRow key={c.id}>
                      <TableCell className="font-medium">{p?.name ?? "Producto"}</TableCell>
      ...
      {scanOpen && (
        <LiveScannerView
          ...
          onDetect={(result) => void onDetected(result)}
          onClose={() => setScanOpen(false)}
          onError={(error) => console.error("Live scanner error", error)}
        />
      )}
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Nuevo código / SKU</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button disabled={loading || !canWrite} onClick={() => void save()}>
