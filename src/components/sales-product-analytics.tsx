...
export function SalesProductAnalytics({ sales, products }: SalesProductAnalyticsProps) {
  ...
  const categories = useMemo(
    () =>
      Array.from(
        new Set((products ?? []).map((p) => p.category).filter((v): v is string => Boolean(v))),
      ).sort(),
    [products],
  );
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Identifica qué productos están moviendo unidades, ingresos y margen.
            </p>
          </div>
          {hasActiveFilters && (
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={`${row.name}-${index}`} className="border-t">
                    ...
                    <td className="px-4 py-3 text-right tabular-nums">
                      {row.margin == null ? "—" : fmtCLP(row.margin)}
                    </td>
                  </tr>
                ))}
              </tbody>
