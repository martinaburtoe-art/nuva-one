import { AlertTriangle, ArrowUpRight, CheckCircle2, CircleDollarSign, Package, Users, Zap, FileText } from "lucide-react";
...
export function NuvaExecutiveCommandCenter({ customers = [], sales = [], activities = [], quotes = [], products = [], executionScore = 0 }: Props) {
  ...
  const urgency = overdue > 0 ? "Atención inmediata" : pendingQuotes.length > 0 ? "Oportunidad comercial" : openTasks > 0 ? "Mantén el foco" : "Operación bajo control";
      ...
      <Card className="overflow-hidden border-primary/20 bg-gradient-to-br from-primary/[0.08] via-background to-background">
        <div className="p-6 md:p-8">
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="max-w-2xl"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><Zap className="h-4 w-4" /> Nüva Executive Intelligence</div><h2 className="mt-2 text-2xl font-semibold tracking-tight md:text-3xl">Nüva encontró lo que merece tu atención.</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Una lectura ejecutiva basada en señales disponibles del negocio, con diagnóstico explicable y siguiente acción.</p></div>
