...
const score = (value: number, target: number) =>
  Math.max(0, Math.min(100, Math.round((value / Math.max(target, 1)) * 100)));
const healthLabel = (value: number) =>
  value >= 80 ? "Saludable" : value >= 60 ? "Estable" : value >= 40 ? "Atención" : "Crítico";
...
export function NuvaExecutiveCommandCenter({
  ...
  const inventory = products.length
    ? Math.max(0, Math.round(100 - (lowStock / products.length) * 100))
    : 0;
  const execution =
    openTasks === 0 ? 100 : Math.max(0, Math.round(100 - (overdue / openTasks) * 100));
  const health = Math.round(
    commercial * 0.3 + customerHealth * 0.2 + inventory * 0.2 + execution * 0.3,
  );
  ...
  const candidates = [
    {
      key: "overdue",
      label: "Resolver seguimientos vencidos",
      reason: `${overdue} seguimiento${overdue === 1 ? "" : "s"} vencido${overdue === 1 ? "" : "s"}`,
      impact: Math.min(100, 45 + overdue * 12),
      urgency: overdue > 0 ? 100 : 0,
    },
    {
      key: "quotes",
      label: "Priorizar cotizaciones abiertas",
      reason: `${pendingQuotes.length} cotización${pendingQuotes.length === 1 ? "" : "es"} abierta${pendingQuotes.length === 1 ? "" : "s"}`,
      impact: Math.min(100, pendingQuoteValue > 0 ? 55 + Math.log10(pendingQuoteValue + 1) * 4 : 0),
      urgency: pendingQuotes.length > 0 ? 65 : 0,
    },
    {
      key: "stock",
      label: "Revisar productos bajo mínimo",
      reason: `${lowStock} producto${lowStock === 1 ? "" : "s"} en nivel mínimo`,
      impact: Math.min(100, lowStock * 18),
      urgency: lowStock > 0 ? 75 : 0,
    },
  ]
    .map((x) => ({ ...x, priority: Math.round(x.impact * 0.65 + x.urgency * 0.35) }))
  ...
  const urgency =
    best.key === "overdue"
      ...
      : best.key === "stock"
        ? "Riesgo operativo"
        : best.key === "quotes" && best.priority > 0
          ? "Oportunidad comercial"
          : openTasks > 0
            ? "Mantén el foco"
            : "Operación bajo control";
  const diagnosis =
    best.key === "overdue" && overdue > 0
      ? `${best.reason}. Nüva prioriza esta acción porque su urgencia es alta y puede afectar la continuidad comercial.`
      : best.key === "stock" && lowStock > 0
        ? `${best.reason}. La prioridad aumenta porque comprometer ventas con stock bajo puede generar fricción operativa.`
        : best.key === "quotes" && pendingQuotes.length > 0
          ? `${best.reason} por ${money(pendingQuoteValue)}. Nüva eleva esta oportunidad por su valor comercial disponible.`
          : "No aparecen señales críticas suficientes para elevar una acción urgente.";
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Estado, evidencia, impacto y decisión en una sola vista ejecutiva.
              </p>
            </div>
            <div className="min-w-36 rounded-2xl border bg-background/80 px-5 py-4 text-center shadow-sm">
              <div className="flex items-center justify-center gap-1.5 text-xs text-muted-foreground">
                <Gauge className="h-3.5 w-3.5" /> Business Health
              </div>
              <div className="mt-1 text-3xl font-bold tabular-nums">{health}</div>
              <div className="text-xs font-medium">{healthLabel(health)} · /100</div>
            </div>
          </div>
