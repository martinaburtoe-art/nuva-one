...
export function NuvaExecutiveCommandCenter({ customers = [], sales = [], activities = [], quotes = [], products = [], executionScore = 0 }: Props) {
  ...
  const risk = overdue > 0 || lowStock > 0;
  const urgency = overdue > 0 ? "Atención inmediata" : lowStock > 0 ? "Riesgo operativo" : pendingQuotes.length > 0 ? "Oportunidad comercial" : openTasks > 0 ? "Mantén el foco" : "Operación bajo control";
  const diagnosis = overdue > 0 ? `Tienes ${overdue} seguimiento${overdue === 1 ? "" : "s"} vencido${overdue === 1 ? "" : "s"}. Resuélvelo antes de crear nuevos seguimientos.` : lowStock > 0 ? `${lowStock} producto${lowStock === 1 ? "" : "s"} está${lowStock === 1 ? "" : "n"} en nivel mínimo. Revisa inventario antes de comprometer nuevas ventas.` : pendingQuotes.length > 0 ? `Hay ${pendingQuotes.length} cotización${pendingQuotes.length === 1 ? "" : "es"} abierta${pendingQuotes.length === 1 ? "" : "s"} por ${money(pendingQuoteValue)}.` : "No aparecen señales críticas en las fuentes operativas disponibles.";
  const action = overdue > 0 ? "Resolver seguimientos vencidos" : lowStock > 0 ? "Revisar productos bajo mínimo" : pendingQuotes.length > 0 ? "Priorizar cotizaciones abiertas" : "Revisar prioridades del CRM";
  ...
  return <section className="space-y-5">
    <Card className="overflow-hidden border-primary/20 bg-gradient-to-br from-primary/[0.08] via-background to-background"><div className="p-6 md:p-8">
      <div className="flex flex-wrap items-start justify-between gap-6"><div className="max-w-2xl"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><Zap className="h-4 w-4" /> Nüva Executive Intelligence</div><h2 className="mt-2 text-2xl font-semibold tracking-tight md:text-3xl">Nüva encontró lo que merece tu atención.</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Estado, evidencia y decisión en una sola vista ejecutiva.</p></div><div className="rounded-2xl border bg-background/80 px-5 py-4 text-center shadow-sm"><div className="text-xs text-muted-foreground">Business Health</div><div className="mt-1 text-3xl font-bold tabular-nums">{health}</div><div className="text-xs font-medium">/ 100</div></div></div>
    ...
    <div className="grid gap-5 lg:grid-cols-[1.35fr_.65fr]"><Card><div className="p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Nüva Focus</p><h3 className="mt-1 text-xl font-semibold">{urgency}</h3></div>{risk ? <ShieldAlert className="h-5 w-5" /> : <ArrowUpRight className="h-5 w-5 text-primary" />}</div><div className="mt-5 rounded-xl border bg-muted/30 p-4"><p className="text-sm font-semibold">{diagnosis}</p><p className="mt-1 text-sm text-muted-foreground">Nüva prioriza la señal más accionable disponible y evita conclusiones sin evidencia.</p></div></div></Card>
      <Card><div className="p-6"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Decision Engine</p><h3 className="mt-2 text-lg font-semibold">{action}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Convierte la señal prioritaria en una acción operativa.</p><Link to="/customer-action-center" className="mt-4 inline-flex items-center text-sm font-semibold text-primary">Ejecutar en Action Center <ArrowUpRight className="ml-1 h-4 w-4" /></Link></div></Card></div>
