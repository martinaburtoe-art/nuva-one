...
select has_function_privilege('authenticated','public.calculate_people_payroll_period(uuid)'::regprocedure,'EXECUTE', 'authenticated can execute payroll calculation');
select has_function_privilege('authenticated','public.prepare_people_lre(uuid)'::regprocedure,'EXECUTE', 'authenticated can prepare LRE');
select has_function_privilege('authenticated','public.generate_people_liquidations(uuid)'::regprocedure,'EXECUTE', 'authenticated can generate liquidations');
select has_function_privilege('authenticated','public.post_people_payroll_to_finance(uuid)'::regprocedure,'EXECUTE', 'authenticated can hand off payroll to finance');
