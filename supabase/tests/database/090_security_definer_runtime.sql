...
select exists (
  select 1
   ...
   where n.nspname = 'public'
     and p.proname = 'create_product_from_scanner'
     and p.prosecdef
     and p.proconfig @> array['search_path=pg_catalog, public, extensions, pg_temp']
) as ok, 'scanner onboarding pins SECURITY DEFINER search_path' as description;
...
select exists (
  select 1
    ...
    join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public'
     and p.proname = 'record_cash_register_movement'
     and p.prosecdef
     and p.proconfig @> array['search_path=pg_catalog, public, extensions, pg_temp']
) as ok, 'cash register mutation pins SECURITY DEFINER search_path' as description;
