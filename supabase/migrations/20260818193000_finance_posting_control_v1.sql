...
CREATE POLICY financial_posting_queue_select ON public.financial_posting_queue
...
CREATE POLICY financial_posting_queue_write ON public.financial_posting_queue
FOR ALL TO authenticated USING (private.has_business_role(business_id, auth.uid(), ARRAY['owner','admin','staff']::public.member_role[]))
WITH CHECK (private.has_business_role(business_id, auth.uid(), ARRAY['owner','admin','staff']::public.member_role[]));
...
BEGIN
  ...
  ON CONFLICT (business_id,source_type,source_id) DO UPDATE
    SET source_date=excluded.source_date,gross_amount=excluded.gross_amount,updated_at=now()
    WHERE financial_posting_queue.status IN ('pending','blocked')
...
CREATE TRIGGER trg_queue_sale_financial_posting
AFTER INSERT OR UPDATE OF status,total ON public.sales
FOR EACH ROW EXECUTE FUNCTION public.enqueue_sale_financial_posting();

DROP TRIGGER IF EXISTS trg_queue_purchase_financial_posting ON public.purchases;
CREATE TRIGGER trg_queue_purchase_financial_posting
AFTER INSERT OR UPDATE OF status,total ON public.purchases
FOR EACH ROW EXECUTE FUNCTION public.enqueue_purchase_financial_posting();

-- Existing operational records are surfaced for controlled review rather than silently posted.
INSERT INTO public.financial_posting_queue(business_id,source_type,source_id,source_date,gross_amount,reason)
SELECT s.business_id,'sale',s.id,s.sale_date,s.total,'Registro histórico pendiente de clasificación contable/tributaria'
FROM public.sales s WHERE s.status <> 'cancelled' AND COALESCE(s.total,0)>0 AND s.accounting_journal_id IS NULL
ON CONFLICT (business_id,source_type,source_id) DO NOTHING;

INSERT INTO public.financial_posting_queue(business_id,source_type,source_id,source_date,gross_amount,reason)
SELECT p.business_id,'purchase',p.id,p.purchase_date,p.total,'Registro histórico pendiente de clasificación contable/tributaria'
FROM public.purchases p WHERE p.status <> 'cancelled' AND COALESCE(p.total,0)>0 AND p.accounting_journal_id IS NULL
ON CONFLICT (business_id,source_type,source_id) DO NOTHING;

-- Treasury projection: historical operational cash movements plus known tax obligations.
CREATE OR REPLACE VIEW public.v_financial_treasury_daily WITH (security_invoker=true) AS
WITH tx AS (
  SELECT business_id, tx_date AS flow_date,
    SUM(CASE WHEN type='income' THEN amount ELSE 0 END)::numeric(18,2) inflow,
    SUM(CASE WHEN type='expense' THEN amount ELSE 0 END)::numeric(18,2) outflow
  FROM public.transactions GROUP BY business_id,tx_date
), tax AS (
  SELECT business_id,due_date AS flow_date,
    0::numeric(18,2) inflow,
    SUM(GREATEST(0,amount-COALESCE(paid_amount,0)))::numeric(18,2) outflow
  FROM public.tax_payments
  WHERE status IN ('planned','due','overdue','partial')
  GROUP BY business_id,due_date
), flows AS (
  SELECT * FROM tx UNION ALL SELECT * FROM tax
)
SELECT business_id,flow_date,SUM(inflow)::numeric(18,2) inflow,SUM(outflow)::numeric(18,2) outflow,
  (SUM(inflow)-SUM(outflow))::numeric(18,2) net_flow
...
SELECT q.business_id,
 COUNT(*) FILTER (WHERE q.status='pending')::integer pending_count,
 COUNT(*) FILTER (WHERE q.status='blocked')::integer blocked_count,
 COUNT(*) FILTER (WHERE q.status='posted')::integer posted_count,
 COALESCE(SUM(q.gross_amount) FILTER (WHERE q.status IN ('pending','blocked')),0)::numeric(18,2) pending_amount,
 MAX(q.created_at) last_queued_at
FROM public.financial_posting_queue q GROUP BY q.business_id;
