...
create index if not exists pricing_calculations_business_created_idx
  on public.pricing_calculations(business_id, created_at desc);
create index if not exists pricing_calculations_business_product_idx
  on public.pricing_calculations(business_id, product_id);
...
create policy pricing_calculations_select_member
...
create policy pricing_calculations_insert_operator
  on public.pricing_calculations for insert
  with check (private.has_business_role(business_id, auth.uid(), ARRAY['owner'::member_role,'admin'::member_role,'staff'::member_role]));

drop policy if exists pricing_calculations_update_operator on public.pricing_calculations;
create policy pricing_calculations_update_operator
  on public.pricing_calculations for update
  using (private.has_business_role(business_id, auth.uid(), ARRAY['owner'::member_role,'admin'::member_role,'staff'::member_role]))
  with check (private.has_business_role(business_id, auth.uid(), ARRAY['owner'::member_role,'admin'::member_role,'staff'::member_role]));

drop policy if exists pricing_calculations_delete_operator on public.pricing_calculations;
create policy pricing_calculations_delete_operator
...
for each row execute function public.set_pricing_calculation_updated_at();
