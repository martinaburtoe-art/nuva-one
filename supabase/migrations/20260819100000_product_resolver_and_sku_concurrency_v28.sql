...
AS $$
  WITH requested AS (
    SELECT lower(btrim(p_code)) AS code
    WHERE p_code IS NOT NULL AND length(btrim(p_code)) > 0
  )
  SELECT
    p.id,
    p.business_id,
    p.name,
    p.sku,
    COALESCE(pc.code, NULLIF(btrim(p.barcode), '')),
    COALESCE(pc.code_type, 'barcode'),
    p.stock,
    p.price,
    p.cost
  ...
  LEFT JOIN public.product_codes pc
   ...
   AND pc.is_active
   AND lower(btrim(pc.code)) = r.code
  WHERE public.is_business_member(p.business_id, (select auth.uid()))
    AND (
      pc.id IS NOT NULL
      OR lower(btrim(COALESCE(p.barcode, ''))) = r.code
      OR lower(btrim(COALESCE(p.sku, ''))) = r.code
    )
  ORDER BY
    CASE
      WHEN lower(btrim(COALESCE(p.sku, ''))) = r.code THEN 0
      WHEN pc.id IS NOT NULL AND pc.is_primary THEN 1
      WHEN pc.id IS NOT NULL THEN 2
      ELSE 3
    END,
    p.id
  LIMIT 2;
...
-- Generate the next SKU under an advisory transaction lock so two concurrent
-- users cannot receive the same candidate. The business is derived from the
-- caller's membership, not from a client-supplied tenant id.
...
BEGIN
  ...
  IF v_business IS NULL THEN
    RAISE EXCEPTION 'No existe un negocio activo para el usuario' USING ERRCODE = '42501';
  END IF;

  -- Advisory lock is transaction-scoped and released automatically at commit/rollback.
...
-- Defense-in-depth: normalize blank SKUs consistently for uniqueness checks.
CREATE UNIQUE INDEX IF NOT EXISTS ux_products_business_sku_nonblank
