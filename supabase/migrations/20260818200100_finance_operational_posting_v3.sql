...
ALTER TABLE public.sales DROP CONSTRAINT IF EXISTS sales_vat_rate_check;
ALTER TABLE public.purchases DROP CONSTRAINT IF EXISTS purchases_vat_rate_check;
...
DECLARE
  s record; v_journal uuid; v_net numeric(18,2); v_vat_amount numeric(18,2); v_counter_key text; v_counter uuid; v_sales uuid; v_vat_account uuid;
BEGIN
  ...
  IF NOT private.has_business_role(s.business_id, auth.uid(), ARRAY['owner','admin','staff']::public.member_role[]) THEN RAISE EXCEPTION 'business write access denied'; END IF;
  ...
  IF s.tax_treatment='taxable' AND s.vat_rate > 0 THEN v_net := round(s.total/(1+s.vat_rate/100),2); v_vat_amount := round(s.total-v_net,2); IF v_vat_account IS NULL THEN RAISE EXCEPTION 'missing VAT debit account'; END IF;
  ELSE v_net := round(s.total,2); v_vat_amount := 0; END IF;
  IF COALESCE(s.is_credit,false) THEN v_counter_key := 'accounts_receivable';
  ELSIF lower(COALESCE(s.payment_method,'')) IN ('transfer','transbank','tarjeta','tarjeta_debito','tarjeta_credito','mercadopago') THEN v_counter_key := 'bank';
  ELSE v_counter_key := 'cash'; END IF;
  ...
  v_journal := public.post_financial_journal(s.business_id, COALESCE(s.sale_date,current_date), 'Venta automática · ' || COALESCE(s.customer_name,'Cliente'), 'sale', s.id,
    CASE WHEN v_vat_amount > 0 THEN jsonb_build_array(
      ...
      jsonb_build_object('account_id',v_vat_account,'debit',0,'credit',v_vat_amount,'description','IVA débito fiscal'))
    ELSE jsonb_build_array(
      jsonb_build_object('account_id',v_counter,'debit',round(s.total,2),'credit',0,'description','Cobro/cliente'),
      jsonb_build_object('account_id',v_sales,'debit',0,'credit',v_net,'description','Venta')) END);
...
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  p record; v_journal uuid; v_net numeric(18,2); v_vat_amount numeric(18,2); v_debit uuid; v_vat_account uuid; v_payable uuid; v_system_key text;
BEGIN
  ...
  IF NOT private.has_business_role(p.business_id, auth.uid(), ARRAY['owner','admin','staff']::public.member_role[]) THEN RAISE EXCEPTION 'business write access denied'; END IF;
  ...
  IF p.tax_treatment='taxable' AND p.vat_rate > 0 THEN v_net := round(p.total/(1+p.vat_rate/100),2); v_vat_amount := round(p.total-v_net,2); IF v_vat_account IS NULL THEN RAISE EXCEPTION 'missing VAT credit account'; END IF;
  ELSE v_net := round(p.total,2); v_vat_amount := 0; END IF;
  v_journal := public.post_financial_journal(p.business_id, COALESCE(p.purchase_date,current_date), 'Compra automática · ' || COALESCE(p.supplier_name,'Proveedor'), 'purchase', p.id,
    CASE WHEN v_vat_amount > 0 THEN jsonb_build_array(
      jsonb_build_object('account_id',v_debit,'debit',v_net,'credit',0,'description',COALESCE(p.category,'Compra')),
      jsonb_build_object('account_id',v_vat_account,'debit',v_vat_amount,'credit',0,'description','IVA crédito fiscal'),
      jsonb_build_object('account_id',v_payable,'debit',0,'credit',round(p.total,2),'description','Proveedor'))
    ELSE jsonb_build_array(
      jsonb_build_object('account_id',v_debit,'debit',v_net,'credit',0,'description',COALESCE(p.category,'Compra')),
      jsonb_build_object('account_id',v_payable,'debit',0,'credit',round(p.total,2),'description','Proveedor')) END);
...
BEGIN
  IF TG_OP='INSERT' OR NEW.status IS DISTINCT FROM OLD.status OR NEW.accounting_journal_id IS NULL THEN PERFORM public.post_sale_accounting(NEW.id); END IF;
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN UPDATE public.sales SET accounting_posting_status='error', accounting_posting_error=left(SQLERRM,500) WHERE id=NEW.id; RETURN NEW; END; $$;

CREATE OR REPLACE FUNCTION public.trg_post_purchase_accounting()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
BEGIN
  IF TG_OP='INSERT' OR NEW.status IS DISTINCT FROM OLD.status OR NEW.accounting_journal_id IS NULL THEN PERFORM public.post_purchase_accounting(NEW.id); END IF;
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN UPDATE public.purchases SET accounting_posting_status='error', accounting_posting_error=left(SQLERRM,500) WHERE id=NEW.id; RETURN NEW; END; $$;

DROP TRIGGER IF EXISTS sales_accounting_auto_post ON public.sales;
CREATE TRIGGER sales_accounting_auto_post AFTER INSERT OR UPDATE OF status,total,payment_method,is_credit,tax_treatment,vat_rate ON public.sales FOR EACH ROW EXECUTE FUNCTION public.trg_post_sale_accounting();
DROP TRIGGER IF EXISTS purchases_accounting_auto_post ON public.purchases;
CREATE TRIGGER purchases_accounting_auto_post AFTER INSERT OR UPDATE OF status,total,category,tax_treatment,vat_rate ON public.purchases FOR EACH ROW EXECUTE FUNCTION public.trg_post_purchase_accounting();
...
FROM (SELECT business_id, accounting_posting_status FROM public.sales UNION ALL SELECT business_id, accounting_posting_status FROM public.purchases) x
