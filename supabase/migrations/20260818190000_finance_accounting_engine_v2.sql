...
DECLARE n integer;
BEGIN
  IF NOT private.is_business_member(p_business_id, auth.uid()) THEN
...
BEGIN
  ...
  VALUES(p_business_id,p_entry_date,p_description,coalesce(p_source_type,'manual'),p_source_id,'posted',auth.uid()) RETURNING id INTO v_journal;
...
-- Seed accounts directly during migration; no auth context is required for migrations.
INSERT INTO public.accounting_accounts(business_id,code,name,account_type,tax_category,system_key)
SELECT b.id,x.code,x.name,x.account_type,x.tax_category,x.system_key FROM public.businesses b CROSS JOIN (VALUES
('1.01.01','Caja','asset','cash','cash'),('1.01.02','Bancos','asset','bank','bank'),('1.01.03','Clientes por cobrar','asset','receivable','accounts_receivable'),('1.01.04','IVA crédito fiscal','asset','vat_credit','vat_credit'),('1.02.01','Inventarios','asset','inventory','inventory'),('1.03.01','Activos fijos','asset','fixed_asset','fixed_assets'),('2.01.01','Proveedores','liability','payable','accounts_payable'),('2.01.02','IVA débito fiscal','liability','vat_debit','vat_debit'),('2.01.03','IVA por pagar','liability','vat_payable','vat_payable'),('2.01.04','PPM por pagar','liability','ppm','ppm_payable'),('3.01.01','Capital','equity','equity','equity'),('4.01.01','Ventas','revenue','sales','sales_revenue'),('5.01.01','Costo de ventas','cost_of_sales','cogs','cost_of_sales'),('6.01.01','Gastos operacionales','expense','operating_expense','operating_expense'),('7.01.01','Otros ingresos','other_income','other_income','other_income'),('8.01.01','Otros gastos','other_expense','other_expense','other_expense')) AS x(code,name,account_type,tax_category,system_key)
ON CONFLICT (business_id,code) DO UPDATE SET name=excluded.name,system_key=excluded.system_key;

CREATE OR REPLACE VIEW public.v_financial_pnl_monthly WITH (security_invoker=true) AS
SELECT j.business_id,date_trunc('month',j.entry_date)::date AS month,
COALESCE(sum(CASE WHEN a.account_type='revenue' THEN l.credit-l.debit ELSE 0 END),0)::numeric(18,2) revenue,
COALESCE(sum(CASE WHEN a.account_type='cost_of_sales' THEN l.debit-l.credit ELSE 0 END),0)::numeric(18,2) cost_of_sales,
COALESCE(sum(CASE WHEN a.account_type='expense' THEN l.debit-l.credit ELSE 0 END),0)::numeric(18,2) operating_expenses,
COALESCE(sum(CASE WHEN a.account_type='other_income' THEN l.credit-l.debit ELSE 0 END),0)::numeric(18,2) other_income,
COALESCE(sum(CASE WHEN a.account_type='other_expense' THEN l.debit-l.credit ELSE 0 END),0)::numeric(18,2) other_expenses,
(COALESCE(sum(CASE WHEN a.account_type='revenue' THEN l.credit-l.debit ELSE 0 END),0)-COALESCE(sum(CASE WHEN a.account_type='cost_of_sales' THEN l.debit-l.credit ELSE 0 END),0)-COALESCE(sum(CASE WHEN a.account_type='expense' THEN l.debit-l.credit ELSE 0 END),0)+COALESCE(sum(CASE WHEN a.account_type='other_income' THEN l.credit-l.debit ELSE 0 END),0)-COALESCE(sum(CASE WHEN a.account_type='other_expense' THEN l.debit-l.credit ELSE 0 END),0))::numeric(18,2) net_result
FROM public.accounting_journals j JOIN public.accounting_lines l ON l.journal_id=j.id AND l.business_id=j.business_id JOIN public.accounting_accounts a ON a.id=l.account_id AND a.business_id=j.business_id
WHERE j.status='posted' GROUP BY j.business_id,date_trunc('month',j.entry_date)::date;

CREATE OR REPLACE VIEW public.v_financial_source_reconciliation WITH (security_invoker=true) AS
SELECT s.business_id,'sale'::text source_type,s.id source_id,s.sale_date source_date,s.total amount,
CASE WHEN s.status='cancelled' THEN 'cancelled' WHEN s.accounting_journal_id IS NULL THEN 'missing_accounting_entry' ELSE 'linked' END status FROM public.sales s
UNION ALL
SELECT p.business_id,'purchase'::text,p.id,p.purchase_date,p.total amount,
CASE WHEN p.status='cancelled' THEN 'cancelled' WHEN p.accounting_journal_id IS NULL THEN 'missing_accounting_entry' ELSE 'linked' END FROM public.purchases p;
