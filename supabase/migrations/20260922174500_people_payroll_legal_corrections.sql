-- Nüva People — payroll calculation correction 2026.09.22
-- Correct legal SIS rate and statutory overtime hourly divisor.

UPDATE public.people_legal_parameters
SET value_numeric=0.0154, source_url='https://www.spensiones.cl/inf_estadistica/iftafp/2026/03/NE-prv202603.pdf',
    source_reference='SP — SIS vigente enero 2026', notes='Tasa SIS vigente 2026; cargo empleador.'
...
v_overtime numeric; v_income_tax numeric; v_deductions numeric; v_net numeric; v_employer_cost numeric; v_hourly numeric; v_warnings jsonb; v_components jsonb; v_bracket record; v_bracket_found boolean;
BEGIN
...
FROM (SELECT DISTINCT ON(parameter_key) parameter_key,value_numeric,value_text FROM public.people_legal_parameters
WHERE country_code='CL' AND effective_from<=v_period_end AND (effective_to IS NULL OR effective_to>=v_period_start)
ORDER BY parameter_key,effective_from DESC) p;
...
v_sis:=COALESCE(NULLIF(v_param->>'sis_rate','')::numeric,0.0154); v_crp:=CASE WHEN v_period_start>=DATE '2026-08-01' THEN COALESCE(NULLIF(v_param->>'crp_rate','')::numeric,0.009) ELSE 0 END;
v_health:=COALESCE(NULLIF(v_param->>'health_rate','')::numeric,0.07); v_ot:=COALESCE(NULLIF(v_param->>'overtime_surcharge','')::numeric,0.50);
...
SELECT c.* INTO v_contract FROM public.people_contracts c WHERE c.employee_id=v_employee.id AND c.business_id=v_period.business_id AND c.status='active'
AND c.start_date<=v_period_end AND (c.end_date IS NULL OR c.end_date>=v_period_start) ORDER BY c.start_date DESC LIMIT 1;
...
SELECT COALESCE(worker_commission,0) INTO v_afp_commission FROM public.people_afp_rates WHERE country_code='CL' AND afp_name=COALESCE(v_employee.afp_name,'')
AND effective_from<=v_period_end AND (effective_to IS NULL OR effective_to>=v_period_start) ORDER BY effective_from DESC LIMIT 1;
...
IF COALESCE(v_employee.health_plan_uf,0)>0 THEN v_health_deduction:=greatest(round(v_pension_base*v_health,0),round(v_employee.health_plan_uf*v_uf+COALESCE(v_employee.health_additional_clp,0),0));
ELSE v_health_deduction:=round(v_pension_base*v_health,0); v_warnings:=v_warnings||jsonb_build_array('Isapre sin plan UF configurado; se aplicó 7% legal'); END IF;
ELSE v_health_deduction:=round(v_pension_base*v_health,0); END IF;
IF lower(COALESCE(v_contract.contract_type,v_employee.employment_type))='indefinite' THEN v_afc_employee:=round(v_unemployment_base*0.006,0); v_afc_employer:=round(v_unemployment_base*0.024,0);
ELSE v_afc_employee:=0; v_afc_employer:=round(v_unemployment_base*0.03,0); END IF;
v_sis_amount:=round(v_pension_base*v_sis,0); v_crp_amount:=round(v_pension_base*v_crp,0);
SELECT b.* INTO v_bracket FROM public.people_tax_brackets b WHERE b.country_code='CL' AND b.tax_type='IUSC' AND b.period_year=v_period.period_year AND b.period_month=v_period.period_month
AND v_taxable-v_afp_employee-v_health_deduction-v_afc_employee>=b.min_income AND (b.max_income IS NULL OR v_taxable-v_afp_employee-v_health_deduction-v_afc_employee<=b.max_income)
ORDER BY b.min_income DESC LIMIT 1;
v_bracket_found:=FOUND;
IF v_bracket_found THEN v_income_tax:=greatest(0,round((v_taxable-v_afp_employee-v_health_deduction-v_afc_employee)*v_bracket.factor-v_bracket.rebate,0));
ELSE v_warnings:=v_warnings||jsonb_build_array('Tabla IUSC no cargada para el período'); END IF;
...
v_components:=jsonb_build_object('salary',v_contract.salary_amount,'overtime',v_overtime,'taxable_bonus',CASE WHEN v_input_found THEN v_input.taxable_bonus ELSE v_employee.taxable_bonus END,
'non_taxable_bonus',v_non_taxable,'gratification',CASE WHEN v_input_found THEN v_input.gratification_amount ELSE 0 END,'afp_employee',v_afp_employee,'health',v_health_deduction,
'afc_employee',v_afc_employee,'afc_employer',v_afc_employer,'sis_employer',v_sis_amount,'crp_employer',v_crp_amount,'pension_base',v_pension_base,'unemployment_base',v_unemployment_base,
'uf_value',v_uf,'pension_cap',v_pension_cap,'unemployment_cap',v_unemployment_cap,'overtime_hourly_base',v_hourly);
INSERT INTO public.people_payroll_items(business_id,payroll_period_id,employee_id,gross_taxable,gross_non_taxable,deductions,employer_cost_amount,net_pay,overtime_amount,vacation_amount,income_tax,social_security,components,warnings,calculation_version,parameter_snapshot)
VALUES(v_period.business_id,p_payroll_period_id,v_employee.id,v_taxable,v_non_taxable,v_deductions,v_employer_cost,v_net,v_overtime,0,v_income_tax,v_afp_employee+v_health_deduction+v_afc_employee+v_sis_amount+v_crp_amount,v_components,v_warnings,'cl-2026.3',v_param);
END LOOP;
