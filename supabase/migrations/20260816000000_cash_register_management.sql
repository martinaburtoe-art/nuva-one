...
CREATE POLICY "Members see cash registers" ON public.cash_registers FOR SELECT USING (private.is_business_member(business_id, auth.uid()));
DROP POLICY IF EXISTS "Members open cash register" ON public.cash_registers;
CREATE POLICY "Members open cash register" ON public.cash_registers FOR INSERT WITH CHECK (private.has_business_role(business_id, auth.uid(), ARRAY['owner','admin','staff']::public.member_role[]) AND opened_by = auth.uid());
DROP POLICY IF EXISTS "Members close cash register" ON public.cash_registers;
CREATE POLICY "Members close cash register" ON public.cash_registers FOR UPDATE USING (private.has_business_role(business_id, auth.uid(), ARRAY['owner','admin','staff']::public.member_role[])) WITH CHECK (private.has_business_role(business_id, auth.uid(), ARRAY['owner','admin','staff']::public.member_role[]));
DROP POLICY IF EXISTS "Members see cash movements" ON public.cash_register_movements;
CREATE POLICY "Members see cash movements" ON public.cash_register_movements FOR SELECT USING (private.is_business_member(business_id, auth.uid()));
DROP POLICY IF EXISTS "Staff record cash movements" ON public.cash_register_movements;
CREATE POLICY "Staff record cash movements" ON public.cash_register_movements FOR INSERT WITH CHECK (private.has_business_role(business_id, auth.uid(), ARRAY['owner','admin','staff']::public.member_role[]) AND created_by = auth.uid() AND EXISTS (SELECT 1 FROM public.cash_registers r WHERE r.id = cash_register_id AND r.business_id = cash_register_movements.business_id AND r.status = 'open'));
