...
BEGIN
  ...
  IF s.tax_treatment='taxable' AND s.vat_rate > 0 THEN
    ...
    v_vat_amount := round(s.total-v_net,2);
    IF v_vat_account IS NULL THEN RAISE EXCEPTION 'missing VAT debit account'; END IF;
  ELSE
  ...
  v_journal := public.post_financial_journal(
    s.business_id, COALESCE(s.sale_date,current_date), 'Venta automática · ' || COALESCE(s.customer_name,'Cliente'), 'sale', s.id,
    CASE WHEN v_vat_amount > 0 THEN
      jsonb_build_array(
...
BEGIN
  ...
  IF p.tax_treatment='taxable' AND p.vat_rate > 0 THEN
    ...
    v_vat_amount := round(p.total-v_net,2);
    IF v_vat_account IS NULL THEN RAISE EXCEPTION 'missing VAT credit account'; END IF;
  ELSE
  ...
  v_journal := public.post_financial_journal(
    p.business_id, COALESCE(p.purchase_date,current_date), 'Compra automática · ' || COALESCE(p.supplier_name,'Proveedor'), 'purchase', p.id,
    CASE WHEN v_vat_amount > 0 THEN
      jsonb_build_array(
...
BEGIN
  IF TG_OP='INSERT' OR NEW.status IS DISTINCT FROM OLD.status OR NEW.accounting_journal_id IS NULL THEN
...
-- Backfill is intentionally executed only by authenticated application actions; migration execution has no auth.uid().
