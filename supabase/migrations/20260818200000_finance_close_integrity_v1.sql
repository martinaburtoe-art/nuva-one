-- Nüva One: Finance close integrity and accounting control layer
-- Adds period-close guardrails, reconciliation exception states and treasury controls.
...
create index if not exists idx_financial_close_controls_business_period
  on public.financial_close_controls(business_id, period_start, period_end);
...
create policy financial_close_controls_select on public.financial_close_controls
...
create policy financial_close_controls_write on public.financial_close_controls
...
-- Prevent posting into an already closed accounting period.
...
begin
  if exists (
    ...
    where c.business_id = p_business_id
      and c.period_start <= p_entry_date
      and c.period_end >= p_entry_date
      and c.status in ('closed','locked','finalized')
  ) then
...
-- Health view: one row per business with the controls that can prevent closure.
create or replace view public.v_financial_close_health
...
select
  ...
  case
    when coalesce(sum(case when c.status = 'blocked' and c.severity in ('critical','high') then 1 else 0 end),0) > 0 then 'blocked'
    when coalesce(sum(case when c.status in ('pending','warning') then 1 else 0 end),0) > 0 then 'in_review'
    when count(c.id) > 0 then 'ready'
    else 'not_started'
  end as close_readiness
from public.businesses b
left join public.financial_close_controls c on c.business_id = b.id
group by b.id;
