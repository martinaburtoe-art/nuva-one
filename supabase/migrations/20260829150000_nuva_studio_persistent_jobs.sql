...
create unique index if not exists nuva_studio_jobs_business_idempotency_idx
  on public.nuva_studio_jobs (business_id, idempotency_key);
create index if not exists nuva_studio_jobs_business_status_idx
  on public.nuva_studio_jobs (business_id, status, created_at desc);
create index if not exists nuva_studio_jobs_scheduler_idx
  on public.nuva_studio_jobs (status, next_run_at)
...
create index if not exists nuva_studio_job_steps_job_status_idx
  on public.nuva_studio_job_steps (job_id, status, step);
...
create index if not exists nuva_studio_job_callbacks_token_idx
  on public.nuva_studio_job_callbacks (token_hash, status);

create or replace function public.nuva_studio_jobs_set_updated_at()
returns trigger
language plpgsql
security invoker
...
create trigger nuva_studio_jobs_set_updated_at
before update on public.nuva_studio_jobs
for each row execute function public.nuva_studio_jobs_set_updated_at();

create or replace function public.nuva_studio_job_steps_set_updated_at()
returns trigger
language plpgsql
security invoker
set search_path = pg_catalog, public
...
create trigger nuva_studio_job_steps_set_updated_at
before update on public.nuva_studio_job_steps
for each row execute function public.nuva_studio_job_steps_set_updated_at();
