-- Manual accounting journals legitimately have no source document.
-- Keep the existing function identity while making the nullable source and payload
-- parameters callable by generated Supabase types without unsafe application casts.
...
begin
  ...
  if p_source_id is not null then
    select id into v_journal from public.accounting_journals where business_id=p_business_id and source_type=v_source_type and source_id=p_source_id limit 1;
  ...
  on conflict (business_id, source_type, source_id) where source_id is not null and source_type is not null do nothing
  ...
  select p_business_id,v_journal,(x->>'account_id')::uuid,x->>'description',coalesce((x->>'debit')::numeric,0),coalesce((x->>'credit')::numeric,0),x->>'tax_code' from jsonb_array_elements(p_lines) x;
