...
begin
  ...
  d:=replace(d,'''cl-2026.4''','''cl-2026.5''');
  d:=replace(d,'v_warnings jsonb; v_components jsonb; v_bracket record; v_bracket_found boolean;','v_warnings jsonb; v_components jsonb; v_bracket record; v_bracket_found boolean; v_afp_rate_found boolean;');
  d:=replace(d,'select coalesce(worker_commission,0) into v_afp_commission from public.people_afp_rates where country_code=''CL'' and lower(trim(afp_name))=lower(trim(coalesce(v_employee.afp_name,''''))) and effective_from<=v_period_end and (effective_to is null or effective_to>=v_period_start) order by effective_from desc limit 1;','select coalesce(worker_commission,0) into v_afp_commission from public.people_afp_rates where country_code=''CL'' and lower(trim(afp_name))=lower(trim(coalesce(v_employee.afp_name,''''))) and effective_from<=v_period_end and (effective_to is null or effective_to>=v_period_start) order by effective_from desc limit 1; select exists(select 1 from public.people_afp_rates where country_code=''CL'' and lower(trim(afp_name))=lower(trim(coalesce(v_employee.afp_name,''''))) and effective_from<=v_period_end and (effective_to is null or effective_to>=v_period_start)) into v_afp_rate_found; if coalesce(v_employee.pension_status,''active'')=''active'' and not v_afp_rate_found then raise exception ''AFP % has no valid rate for payroll period %/%'',v_employee.afp_name,v_period.period_year,v_period.period_month; end if;');
